...
func noteMonterRoutes(api huma.API, s *service) {
	maxOctets, err := socle.EnvInt("NOTE_VOCALE_MAX_SIZE_BYTES", 25_000_000)
	if err != nil {
		slog.Error("réglage des notes vocales", "err", err)
		maxOctets = 25_000_000
	}
	heures, err := socle.EnvInt("NOTE_VOCALE_RETENTION_HOURS", 48)
	if err != nil {
		slog.Error("réglage des notes vocales", "err", err)
		heures = 48
	}
	noteConfig = noteReglages{
		dir:       socle.Env("NOTE_VOCALE_DIR", "./storage/notes-vocales"),
...
func (s *service) noteAcces(ctx context.Context, attemptID string, depot bool) error {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return socle.Problem(http.StatusNotFound, "CALL_ATTEMPT_NOT_FOUND", "Tentative d’appel introuvable.")
	}
	if err != nil {
		return err
	}
	...
	if !depot && (u.Role == socle.Admin || u.Role == socle.Superviseur) {
		return nil
	}
	return socle.Problem(http.StatusForbidden, "NOTE_VOCALE_INTERDITE", "Cette note vocale ne vous appartient pas.")
}
...
func (s *service) noteLire(ctx context.Context, in *QualificationIDInput) (*huma.StreamResponse, error) {
	...
	if err != nil {
		return nil, socle.Problem(http.StatusNotFound, "NOTE_VOCALE_INTROUVABLE", "Aucune note vocale pour cette tentative.")
	}
	info, err := f.Stat()
	if err != nil {
