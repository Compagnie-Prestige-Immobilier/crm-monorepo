...
func dumpActif() error {
	if socle.Env("DB_DUMP_ENABLED", "false") == socle.Vrai {
		return nil
	}
	return socle.Problem(http.StatusNotFound, "DATABASE_DUMP_DISABLED", "L’export intégral n’est pas activé sur ce déploiement.")
}

func repertoireDump() string {
	return socle.Env("DB_DUMP_DIR", "./storage/db-dumps")
}
...
func (s *service) aviserExportPret(ctx context.Context, t *travailDump, acteurID string) {
	_, err := notifications.Composer(ctx, s.Deps, acteurID, &notifications.CreationNotification{
...
func (s *service) demanderDump(ctx context.Context, _ *struct{}) (*EtatDumpOutput, error) {
	...
	if existe && courant.Status == dumpPret {
		return nil, socle.Problem(http.StatusConflict, "DATABASE_DUMP_ALREADY_READY",
	...
	if err := database.Auditer(ctx, s.Q, acteur.ID, "DATABASE_DUMP_REQUESTED", "database", acteur.ID, nil, map[string]any{"jobId": t.ID}); err != nil {
		return nil, err
	}
	balayerOrphelins("")
...
func (s *service) oublierFichierDump(ctx context.Context, t *travailDump) error {
	...
	return socle.Problem(http.StatusNotFound, pasPret, "Le fichier d’export n’est plus disponible. Relancez un export.")
}
...
func (s *service) consommerDump(ctx context.Context, t *travailDump, acteurID, nom string, taille int64) {
	if err := database.Auditer(ctx, s.Q, acteurID, "DATABASE_DUMP_DOWNLOADED", "database", acteurID, nil,
