...
async function attendreRefus(page: Page, role: RoleCompte, route: string): Promise<void> {
  ...
  const ou = `${route} pour ${role}`;
  await expect(refus.getByRole('heading', { name: 'Accès refusé', level: 2 }), ou).toBeVisible();
  await expect(refus, ou).toContainText(`Rôle en cours : ${LIBELLE[role]}.`);
  const sortie = refus.getByRole('link', { name: 'Retour à l’accueil', exact: true });
  await expect(sortie, ou).toHaveAttribute('href', '/espaces');
}
...
test.describe('parité accès, la connexion, la session et ses destinations', () => {
  ...
  test('la destination demandée est tenue, la déconnexion la reverrouille', async ({
    ...
    const pose = (await context.cookies()).find((c) => c.name === '__Host-cpi_session');
    expect(pose?.httpOnly, 'une XSS suffirait à voler la session').toBe(true);
  ...
  test('une destination hostile, « //ailleurs » ou « /\\ailleurs », reste sur l’origine', async ({
    ...
    const hors = visitees.filter(
      (url) => url !== 'about:blank' && new URL(url).origin !== new URL(BASE_URL).origin,
    );
    expect(hors, 'le panneau ne doit jamais servir de tremplin de redirection').toEqual([]);
  });

  test('une session déjà ouverte reprend la main sur /connexion', async ({ browser }) => {
    const page = await pageDe(browser, 'ADMIN');
    await page.goto('/connexion');

    await expect(page).toHaveURL(/\/espaces$/);
    await expect(page.getByRole('heading', { name: 'Vos espaces', level: 1 })).toBeVisible();
    await expect(page, 'le titre du document est posé par l’écran').toHaveTitle(
      'Vos espaces · CPI GO',
    );
    await page.context().close();
  });

  test('le hub propose « Revenir » vers l’écran quitté, jamais vers lui-même', async ({
    browser,
  }) => {
    const page = await pageDe(browser, 'ADMIN');
    const revenir = page.getByRole('link', { name: 'Revenir' });

    await page.goto('/espaces?retour=%2Faccueil%2Flistes');
    await expect(revenir).toHaveAttribute('href', '/accueil/listes');

    await page.goto('/espaces?retour=%2Fespaces');
    await expect(page.getByRole('heading', { name: 'Vos espaces', level: 1 })).toBeVisible();
    await expect(revenir, 'un retour vers le hub ne mène nulle part').toHaveCount(0);
    await page.context().close();
  });
...
test('hors session, chaque adresse du panneau renvoie sur la connexion sans rien laisser voir', async ({
  ...
  for (const route of ['/accueil', '/chues/prospects', '/admin/commerciaux', '/notifications']) {
    ...
    await expect(page).toHaveTitle('Connexion · CPI GO');
    await expect(
...
test.describe('parité rôles, le refus est lisible et rien ne se charge', () => {
  ...
  test('aucun écran ne se referme sur l’administrateur', async ({ browser }) => {
    ...
    for (const { route } of ECRANS) {
      await page.goto(route);
      // Le titre du document est posé par l'écran : « CPI GO · CPI GO » est le
      // repli d'une route qui ne correspond à aucune entrée de navigation.
      await expect(page, `${route} n’a pas rendu son écran`).not.toHaveTitle('CPI GO · CPI GO');
      await expect(page, `${route} n’a pas rendu son écran`).toHaveTitle(/ · CPI GO$/);
      await expect(
  ...
  test('le composeur d’administration renvoie les rôles à boîte et refuse les autres', async ({
    browser,
  }) => {
    test.slow();
    for (const role of ['DIRECTION', 'SUPERVISEUR', 'ACCUEIL', 'BANQUE_FINANCE'] as const) {
      const page = await pageDe(browser, role);
      await page.goto('/admin/notifications');
      await expect(page, `${role} devrait retrouver sa boîte`).toHaveURL(/\/notifications$/);
      await expect(
    ...
    for (const role of ['COMMERCIAL', 'CHARGE_CLIENTELE'] as const) {
      const page = await pageDe(browser, role);
      await page.goto('/admin/notifications');
      await attendreRefus(page, role, '/admin/notifications');
      await expect(page, 'un refus ne déplace pas l’adresse').toHaveURL(/\/admin\/notifications$/);
      await page.context().close();
    }
  });
});
