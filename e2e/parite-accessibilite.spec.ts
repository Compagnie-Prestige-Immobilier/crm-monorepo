...
const ECRANS: readonly { role: RoleCompte; chemin: string; repere: string }[] = [
  ...
  { role: 'ADMIN', chemin: '/chues/prospects?search=E2E%20a11y', repere: NOM_PROSPECT_CHUES },
  { role: 'ADMIN', chemin: '/chues/representants', repere: NOM_REPRESENTANT },
  { role: 'ADMIN', chemin: '/grand-public?search=E2E%20a11y', repere: NOM_PROSPECT_GP },
  {
    role: 'ADMIN',
...
test.describe('parité accessibilité, tuiles fermées du hub', () => {
  ...
  test('aucune violation axe sur /espaces vu par un compte ACCUEIL', async ({ page }) => {
    await page.emulateMedia({ colorScheme: 'light' });
...
const ECRANS_SOMBRES: readonly { chemin: string; repere: string }[] = [
  { chemin: '/chues/prospects?search=E2E%20a11y', repere: NOM_PROSPECT_CHUES },
  { chemin: '/grand-public?search=E2E%20a11y', repere: NOM_PROSPECT_GP },
  {
    chemin: '/admin/parametres',
    repere: 'Ces actions portent sur les données de tous les utilisateurs.',
  },
...
test.describe('parité accessibilité, titre du document', () => {
  ...
  test('chaque écran nomme l’onglet, hub et panneau compris', async ({ page }) => {
    await page.goto('/espaces');
    await expect(page).toHaveTitle('Vos espaces · CPI GO');
