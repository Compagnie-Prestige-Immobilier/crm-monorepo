...
const VARIANTS = {
  ...
  secondary: 'border border-line bg-paper text-ink data-[hovered]:border-brand/30 data-[hovered]:bg-brand-soft/50',
...
export function Button({ variant, className = '', ...props }: Omit<ButtonProps, 'className'> & { variant?: Variant; className?: string }) {
  return <AriaButton {...props} className={`${buttonClass(variant)} ${className}`} />;
}

type FieldProps = { label: string; value: string; onChange: (value: string) => void; isRequired?: boolean; isInvalid?: boolean; className?: string };
...
function FieldLabel({ label, isRequired }: { label: string; isRequired?: boolean | undefined }) {
  return (
    <Label className="text-[13px] font-semibold text-muted">
      {label}
      {isRequired && <span className="ml-0.5 text-ko" aria-hidden>*</span>}
    </Label>
  );
}
...
const CLEAR = '__clear';
export function SelectField({ label, value, onChange, options, isRequired, isInvalid, className = '' }: FieldProps & { options: readonly Option[] }) {
  return (
    <Select
    ...
    >
      ...
      <Popover className="w-(--trigger-width) min-w-60 rounded-xl border border-line bg-paper p-1 shadow-xl shadow-ink/10">
        <ListBox className="max-h-72 overflow-auto outline-none">
          ...
          {options.map(([key, text]) => <MenuOption key={key} id={key} label={text} />)}
        </ListBox>
      </Popover>
    </Select>
  );
...
export function TextInput({ label, value, onChange, isRequired, isInvalid, className = '', type = 'text', placeholder, multiline }: FieldProps & { type?: string; placeholder?: string | undefined; multiline?: boolean }) {
  return (
    <TextField className={`${fieldClass} ${className}`} value={value} onChange={onChange} isInvalid={isInvalid ?? false} type={type}>
      <FieldLabel label={label} isRequired={isRequired} />
      {multiline ? <TextArea className="field-box min-h-24 py-2.5" placeholder={placeholder} /> : <Input className="field-box" placeholder={placeholder} />}
      <FieldError className={errorClass}>Champ obligatoire</FieldError>
    </TextField>
  );
}

export function NumberInput({ label, value, onChange, isRequired, isInvalid, className = '' }: FieldProps) {
  return (
    <NumberField
      className={`${fieldClass} ${className}`}
...
export function Sheet({ isOpen, onOpenChange, title, children }: { isOpen: boolean; onOpenChange: (open: boolean) => void; title: string; children: ReactNode }) {
  return (
    <ModalOverlay isOpen={isOpen} onOpenChange={onOpenChange} isDismissable className="fixed inset-0 z-50 flex items-end justify-center bg-ink/40 backdrop-blur-[2px] sm:items-center sm:p-6">
      <Modal className="flex max-h-[92dvh] w-full flex-col rounded-t-3xl bg-paper shadow-2xl sm:max-w-2xl sm:rounded-3xl">
        <Dialog className="flex min-h-0 flex-col outline-none">
          <header className="flex items-center justify-between gap-4 border-b border-line px-5 py-4 sm:px-6">
            <Heading slot="title" className="font-display text-xl font-bold">{title}</Heading>
...
export function ClassStamp({ classe, size = 'md' }: { classe: Classe; size?: 'sm' | 'md' | 'lg' }) {
  const dims = { sm: 'size-9 text-lg rounded-md', md: 'size-12 text-2xl rounded-lg', lg: 'size-20 text-5xl rounded-xl' }[size];
  return (
    <span aria-label={`Classe ${classe}`} className={`inline-grid shrink-0 -rotate-6 place-items-center border-2 font-display font-extrabold leading-none outline outline-1 -outline-offset-[5px] outline-current/25 ${dims} ${STAMP[classe]}`}>
      {classe}
    </span>
  );
}

export function Bar({ label, value, max, tone = 'bg-brand' }: { label: string; value: number; max: number; tone?: string }) {
  return (
    <Meter value={value} maxValue={max} aria-label={label} className="grid grid-cols-[6.5rem_1fr_3.5rem] items-center gap-3 text-sm">
      {({ percentage }) => (
        <>
          <span className="text-muted">{label}</span>
          <span className="h-1.5 overflow-hidden rounded-full bg-line">
            <span className={`block h-full rounded-full transition-[width] duration-500 ${tone}`} style={{ width: `${percentage}%` }} />
          </span>
