...
var (
	...
	axeDepartement     = axeDeRepartition{`INNER JOIN "departements" d ON d."id" = r."departementId"`, `d."id"`, colonneDepartementNom}
...
func litToutLeTravail(role Role) bool {
...
func teleconseillerLisible(u *Utilisateur, demande string) string {
...
func erreurDateInvalide(champ string) *ProblemError {
	return problem(http.StatusBadRequest, "BAD_REQUEST", "Date invalide dans "+champ+".")
}
...
func porteeDeLecture(u *Utilisateur, f *FiltreDesAnalyses, p *parametresSQL) []string {
	conditions := []string{}
	if !litToutLeTravail(u.Role) {
	...
	if !f.IncludeDeleted || u.Role != Admin {
...
func filtreSegment(segment string, p *parametresSQL) string {
	...
	if segment == lotSegmentBDD1 || segment == lotSegmentBDD2 {
		syndicat = `sy."sigle" = `
	}
	if segment == lotSegmentBDD1 || segment == lotSegmentBDD3 {
		banque = `bq."shortName" = `
	}
...
func (s *service) filtreRechercheLibre(brut string, p *parametresSQL) string {
	...
	numero := compact
	if e164, err := normaliserTelephone(recherche, s.cfg.PhoneRegion); err == nil {
		numero = e164
	}
...
func (s *service) prospectsParMethodeEnrolement(ctx context.Context, f *FiltreDesAnalyses) (*MethodesEnrolementOutput, error) {
	corps, err := avecCache(cleDeCacheDesAnalyses(ctx, "by-enrollment-method", f), ttlAnalyses, func() (RepartitionParMethodeEnrolement, error) {
		...
		items := make([]CompteParMethodeEnrolement, 0, len(exportOrdreMethodes))
		for _, methode := range exportOrdreMethodes {
			items = append(items, CompteParMethodeEnrolement{
				Method:    methode,
				Label:     exportLibellesMethode[methode],
...
var gardeAnalytics = map[string][]Role{
	"GET /api/v1/analytics/funnel":               parcours,
	"GET /api/v1/analytics/delays":               parcours,
	"GET /api/v1/analytics/departement-yield":    parcours,
	"GET /api/v1/analytics/by-enrollment-method": parcours,
	"GET /api/v1/analytics/by-banque":            parcours,
	"GET /api/v1/analytics/by-departement":       parcours,
	"GET /api/v1/analytics/by-syndicat":          parcours,
	"GET /api/v1/supervision/representants":      encadrement,
	"GET /api/v1/supervision/campagnes":          encadrement,
	"GET /api/v1/supervision/activite":           encadrement,
	"GET /api/v1/supervision/creneaux":           encadrement,
	"PUT /api/v1/supervision/creneaux":           encadrement,
}
...
func monterAnalytics(api huma.API, s *service) {
