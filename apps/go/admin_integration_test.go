...
func TestAdminReinitialisationDuMotDePasse(t *testing.T) {
	...
	statut, body = adminAppel(b, http.MethodPut, "/api/v1/users/"+cibleID+"/password", map[string]any{"password": "nouveau-mdp-2026"})
	...
	statut, body = cible.connexion(cibleEmail, "nouveau-mdp-2026")
...
func TestAdminExportDeLaBaseTelechargePuisDetruit(t *testing.T) {
	...
	t.Cleanup(func() { _, _ = b.pool.Exec(b.ctx, `DELETE FROM "app_settings" WHERE "key" = $1`, cleDump) })
	adminExec(b, `DELETE FROM "app_settings" WHERE "key" = $1`, cleDump)
...
func TestAdminSupervisionLitLaPresenceBattue(t *testing.T) {
	...
	statut, body = adminAppel(b, http.MethodGet, cheminSupervision, nil)
	...
	statut, body = adminAppel(tele, http.MethodGet, cheminSupervision, nil)
...
func adminEnteteSupervision(b *banc, body map[string]any) {
	b.t.Helper()
	if nombreDe(body["onlineWindowMinutes"]) != fenetreConnecteMinutes || body["observedAt"] == nil {
...
func TestAdminCreationEtListeDesComptes(t *testing.T) {
	...
	corps := map[string]any{
		...
		"fullName": "Fatou Ndiaye", "password": "motdepasse-2026", "phone": "77 123 45 67",
	}
...
func TestAdminDispositionParEcranEtParCompte(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = b.pool.Exec(b.ctx, `DELETE FROM "app_settings" WHERE "key" = $1`, cleDispositionDefaut("chues"))
	})
