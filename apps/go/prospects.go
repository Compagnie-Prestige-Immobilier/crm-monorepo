...
const (
	...
	prospectChampEtablissement      = "etablissement"
	prospectChampDureeSysteme       = "dureeSystemeMois"
	prospectChampWhatsappStatut     = "whatsappStatus"
	prospectChampWhatsappNumero     = "whatsappE164"
...
func prospectPorteeDe(u *Utilisateur) prospectPortee {
	return prospectPortee{
		tout:     u.Role == Admin || u.Role == Superviseur || u.Role == Direction,
		converti: u.Role == ChargeClientele,
		userID:   u.ID,
	}
}
...
func (s *service) prospectLire(ctx context.Context, u *Utilisateur, id string) (*Prospect, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, problem(http.StatusNotFound, prospectCodeIntrouvable, prospectIntrouvable)
	}
	if err != nil {
		return nil, err
	}
	return nil, problem(http.StatusForbidden, "NOT_OWNER", "Cette fiche appartient à un autre téléconseiller.")
}

func (s *service) prospectModifiable(ctx context.Context, u *Utilisateur, id string) (db.ProspectVivantRow, error) {
	row, err := s.q.ProspectVivant(ctx, id)
	if errors.Is(err, pgx.ErrNoRows) {
		return row, problem(http.StatusNotFound, prospectCodeIntrouvable, prospectIntrouvable)
	}
	if err != nil {
		return row, err
	}
	if u.Role != Admin && row.CreatedById != u.ID {
		return row, problem(http.StatusForbidden, "NOT_OWNER", "Cette fiche appartient à un autre commercial.")
	}
	return row, nil
}
...
func prospectBorneDate(brut string, zone *time.Location, fin bool) (*time.Time, error) {
	...
	if err != nil {
		return borne, problem(http.StatusBadRequest, prospectCodeMauvaiseRequete, "Date illisible : "+brut)
	}
...
func prospectRechercheTelephone(recherche, region string) *string {
	...
	if e164, err := normaliserTelephone(recherche, region); err == nil {
		return &e164
	}
	return &chiffres
}

func (s *service) prospectFiltres(in *ProspectListInput, u *Utilisateur) (db.ListProspectsParams, error) {
	p := prospectPorteeDe(u)
...
func prospectComptage(arg *db.ListProspectsParams) db.CountProspectsParams {
	return db.CountProspectsParams{
		...
		Search: arg.Search, PhoneSearch: arg.PhoneSearch,
	}
}
...
func prospectPaiementCoherent(mode *string, duree *int32) error {
	if mode != nil && *mode == string(db.PaymentModeCOMPTANT) && duree != nil {
		return problem(http.StatusBadRequest, "PROSPECT_PAYMENT_DURATION_INVALID", "La durée ne concerne que le paiement échelonné.")
	}
	return nil
}
...
func (s *service) prospectNumeroOptionnel(brut *string) (*string, error) {
	...
	normalise, err := normaliserTelephone(*brut, s.cfg.PhoneRegion)
	if err != nil {
		return e164, err
	}
...
func (s *service) prospectCommercialUtilisable(ctx context.Context, id *string) error {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return problem(http.StatusNotFound, "USER_NOT_FOUND", "Commercial de destination introuvable.")
	}
	return err
}
...
func (s *service) prospectTelephoneLibre(ctx context.Context, u *Utilisateur, phoneE164 string, saufID *string) error {
	...
	if u.Role == Admin || clash.CreatedById == u.ID {
	...
	p := problem(http.StatusConflict, "PROSPECT_PHONE_CONFLICT", "Ce numéro a déjà été enregistré par "+clash.OwnerName+".")
	p.Errors = []*huma.ErrorDetail{{Location: "existing", Value: existante}}
	return p
}

func (s *service) prospectIdentifiantLibre(ctx context.Context, u *Utilisateur, id string) error {
	...
	if u.Role != Admin && row.CreatedById != u.ID {
		return problem(http.StatusForbidden, "ENTITY_ID_OWNED_BY_ANOTHER_USER", "Cet identifiant appartient à un autre commercial.")
	}
	p := problem(http.StatusConflict, "PROSPECT_ALREADY_EXISTS", "Un prospect porte déjà cet identifiant.")
	p.Errors = []*huma.ErrorDetail{{Location: cleIdentifiantExistant, Value: id}}
	return p
}
...
func (s *service) prospectAvantCreation(ctx context.Context, u *Utilisateur, corps *ProspectBody) (phoneE164, id string, err error) {
	...
	if phoneE164, err = normaliserTelephone(*corps.Phone, s.cfg.PhoneRegion); err != nil {
		return "", "", err
	}
...
func (s *service) prospectInserer(ctx context.Context, u *Utilisateur, id, phoneE164 string, projet db.Projet, corps *ProspectBody) error {
...
func prospectInstant(brut *string) (time.Time, error) {
	...
	instant, err := time.Parse(time.RFC3339, *brut)
	if err != nil {
		return time.Time{}, problem(http.StatusBadRequest, prospectCodeMauvaiseRequete, "Date de saisie illisible.")
	}
	return instant, nil
}
...
func (s *service) prospectRattacher(ctx context.Context, u *Utilisateur, phoneE164 string, projet db.Projet, corps *ProspectBody) (*ProspectOutput, error) {
	...
	if u.Role != Admin && existant.CreatedById != u.ID {
		return aucune, nil
	}
	journeyID, err := uuid.NewV7()
	if err != nil {
...
func (s *service) prospectMajAutorisee(ctx context.Context, u *Utilisateur, existant *db.ProspectVivantRow, corps *ProspectBody, phoneE164 *string) error {
...
func prospectStatutModifiable(u *Utilisateur, courant db.ProspectStatut, demande *string) error {
	if demande == nil || *demande == string(courant) || u.Role == Admin {
		return nil
	}
	suivant := db.ProspectStatut(*demande)
	if !slices.Contains(prospectTransitions[courant], suivant) {
		return problem(http.StatusForbidden, "PROSPECT_STATUT_TRANSITION_REFUSED",
			"Statut du prospect : le passage de « "+string(courant)+" » à « "+*demande+" » n’est pas permis.")
	}
	if suivant == db.ProspectStatutCONVERTI {
		return problem(http.StatusForbidden, "PROSPECT_CONVERSION_REQUIRES_CONFIRMATION",
...
func (s *service) prospectMajColonnes(maj *prospectMaj, corps *ProspectBody, existant *db.ProspectVivantRow, phoneE164 *string) error {
	...
	prospectPoser(maj, prospectChampPrenom, prospectRogner(corps.Prenom))
	prospectPoser(maj, prospectChampPhone, phoneE164)
	prospectPoser(maj, "representantId", corps.RepresentantID)
	prospectPoser(maj, prospectChampProfession, prospectRogner(corps.Profession))
	...
	prospectPoserOptionnel(maj, prospectChampEtablissement, prospectOptionnelRogne(corps.Etablissement))
...
func (s *service) prospectMajWhatsapp(maj *prospectMaj, corps *ProspectBody, existant *db.ProspectVivantRow, phoneE164 *string) error {
	...
	maj.set(prospectChampWhatsappStatut, *statut)
	maj.set(prospectChampWhatsappNumero, retenu)
	return nil
}
...
func (s *service) prospectSupprimer(ctx context.Context, in *ProspectIDInput) (*ProspectOkOutput, error) {
	...
	avant := map[string]string{
		prospectChampNom: existant.Nom, prospectChampPrenom: existant.Prenom,
		prospectChampPhone: existant.PhoneE164,
	}
	err = s.prospectTx(ctx, func(q *db.Queries) error {
		...
		return auditer(ctx, q, u.ID, "prospect.delete", prospectEntite, in.ID, avant, nil)
	})
	if err != nil {
		return nil, err
	}
...
func (s *service) prospectRevue(ctx context.Context, in *ProspectIDInput) (*ProspectOutput, error) {
	...
	if existant.Statut != db.ProspectStatutCONVERTI {
		return nil, problem(http.StatusBadRequest, "PROSPECT_REVUE_REQUIRES_CONVERSION", "Seule une demande convertie se revoit.")
	}
...
func (s *service) prospectFusionner(ctx context.Context, in *ProspectFusionInput) (*ProspectOutput, error) {
	...
	if corps.SourceID == corps.TargetID {
		return nil, problem(http.StatusBadRequest, "MERGE_SAME_PROSPECT", "Impossible de fusionner une fiche avec elle-même.")
	}
	...
	if ouverts > 1 {
		return nil, problem(http.StatusConflict, "MERGE_TWO_OPEN_BANK_CASES",
...
func (s *service) prospectReaffecter(ctx context.Context, in *ProspectReaffectationInput) (*ProspectReaffectationOutput, error) {
	...
	if corps.RepresentantID == nil && corps.CommercialID == nil {
		return nil, problem(http.StatusBadRequest, "REASSIGN_NO_TARGET", "Indiquez au moins un représentant ou un commercial de destination.")
	}
	if corps.CommercialID != nil && u.Role != Admin {
		return nil, problem(http.StatusForbidden, "REASSIGN_OWNER_FORBIDDEN", "Seul un administrateur peut changer le commercial propriétaire.")
	}
	ids, err := s.q.ProspectsAReaffecter(ctx, db.ProspectsAReaffecterParams{
		Ids: corps.ProspectIDs, ScopeAll: u.Role == Admin, ScopeUserID: u.ID,
	})
	...
	if err := s.prospectTx(ctx, func(q *db.Queries) error {
		...
		return auditer(ctx, q, u.ID, "prospect.reassign", prospectEntite, ids[0], nil,
...
var prospectLecture = []Role{Commercial, ChargeClientele, Admin, Superviseur, Direction}

var gardeProspects = map[string][]Role{
	...
	"POST /api/v1/prospects":                                          parcours,
	"PATCH " + prospectCheminID:                                       parcours,
	"DELETE " + prospectCheminID:                                      parcours,
	"POST /api/v1/prospects/merge":                                    {Commercial, ChargeClientele, Admin},
	"POST /api/v1/prospects/reassign":                                 {Commercial, ChargeClientele, Admin},
	"POST /api/v1/prospects/{id}/revue":                               {ChargeClientele, Superviseur, Admin},
	"PATCH /api/v1/prospects/{id}/parcours/grand-public/consentement": {Commercial, ChargeClientele, Admin},
	"POST /api/v1/prospects/{id}/parcours/grand-public/conversion":    {Commercial, ChargeClientele, Admin},
	"GET /api/v1/champs-conversion/{projet}":                          tous,
	"PUT /api/v1/champs-conversion/{projet}":                          admin,
	"GET /api/v1/parametres-chues":                                    parcours,
	"PATCH /api/v1/parametres-chues":                                  encadrement,
	"GET /api/v1/parametres-chues/journal":                            encadrement,
}

func monterProspects(api huma.API, s *service) {
