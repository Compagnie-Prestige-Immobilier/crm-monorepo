...
func (s *service) listerInscriptions(ctx context.Context, in *ListerInscriptionsInput) (*ListerInscriptionsOutput, error) {
	...
	filtres := db.CountInscriptionsParams{
		Projet: db.Projet(in.Projet), InclureDisparues: in.InclureDisparues == vrai,
...
func inscriptionIntrouvable() error {
	return problem(http.StatusNotFound, "INSCRIPTION_INTROUVABLE", "Cette inscription n’existe pas pour ce projet.")
}
...
func (s *service) tirerEnrolement(ctx context.Context) error {
	for _, projet := range []string{projetChues, projetGrandPublic} {
		base, jeton := plateformeConfiguree(projet)
		if base == "" || jeton == "" {
			continue
		}
...
func lireChues(ctx context.Context, base, jeton string) ([]inscriptionDistante, error) {
	...
	for _, brut := range comptes.Clients {
		...
		lignes = append(lignes, inscriptionDistante{
			...
			PhoneE164:          telephoneOptionnel(ligne.Phone, "SN"),
			Email:              courriel,
...
func pageGrandPublic(ctx context.Context, base, jeton string, page int) (lignes []inscriptionDistante, dernierePage int, err error) {
	...
	if _, err := appelPlateforme(ctx, projetGrandPublic, url, jeton, &reponse); err != nil {
		return nil, page, err
	}
