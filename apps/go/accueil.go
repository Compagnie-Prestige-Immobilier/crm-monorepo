...
var gardeAccueil = map[string][]Role{
	"GET /api/v1/visites/referentiels":        tous,
	"GET /api/v1/visites/statistiques":        registre,
	"GET /api/v1/visites":                     registre,
	"POST /api/v1/visites":                    registre,
	"GET /api/v1/visites/{id}":                registre,
	"PATCH /api/v1/visites/{id}":              registre,
	"POST /api/v1/visites/import":             {Admin, Direction},
	"GET /api/v1/visites/import/{id}":         {Admin, Direction},
	"GET /api/v1/visites/import/{id}/revue":   {Admin, Direction},
	"PATCH /api/v1/visites/import/{id}/revue": {Admin, Direction},
	"POST /api/v1/visites/import/{id}/apply":  {Admin, Direction},
}

func monterAccueil(api huma.API, s *service) {
...
func (s *service) creerVisite(ctx context.Context, in *CreerVisiteInput) (*VisiteOutput, error) {
	...
	params := db.InsererVisiteParams{
		...
		Phone:       texteRegistre(corps.Phone), PhoneE164: telephoneOptionnel(corps.Phone, s.cfg.PhoneRegion),
		EntrepriseId: corps.EntrepriseID, ObjetId: corps.ObjetID,
		DirectionId: corps.DirectionID, DestinataireId: corps.DestinataireID,
		Comment: texteRegistre(corps.Comment), CreatedById: utilisateurCourant(ctx).ID,
	}
	...
	return nil, problem(http.StatusBadRequest, "VISITE_REFERENCE_EXHAUSTED", "La référence de visite n’a pas pu être attribuée. Réessayez.")
}
...
func (s *service) corrigerVisite(ctx context.Context, in *CorrigerVisiteInput) (*VisiteOutput, error) {
	...
	if corps.Phone.fourni {
		...
		params.PhoneE164 = telephoneOptionnel(params.Phone, s.cfg.PhoneRegion)
	}
...
func (s *service) visite(ctx context.Context, id string) (db.VisiteParIdRow, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return row, problem(http.StatusNotFound, "VISITE_NOT_FOUND", "Visite introuvable.")
	}
	return row, err
}
...
func (s *service) verifierReferentiels(ctx context.Context, entreprise, objet, direction, destinataire *string) error {
	...
	for _, choix := range []struct {
		...
		return problem(http.StatusBadRequest, "VISITE_REFERENTIEL_UNAVAILABLE",
...
func (s *service) prochaineReference(ctx context.Context, instant time.Time) (string, error) {
	...
	if rang > referenceMaxRang {
		return "", problem(http.StatusBadRequest, "VISITE_REFERENCE_EXHAUSTED", "La référence de visite n’a pas pu être attribuée. Réessayez.")
	}
...
func periodeStats(du, au string) (debut, fin time.Time, err error) {
	...
	if fin.Before(debut) {
		return debut, fin, problem(http.StatusBadRequest, "VISITE_STATS_RANGE_INVALID", "La date de fin précède la date de début.")
	}
	if fin.Sub(debut) > statsMaxJours*24*time.Hour {
		return debut, fin, problem(http.StatusBadRequest, "VISITE_STATS_RANGE_TOO_WIDE",
