...
var colonnesRepresentantsImport = []colonneImport{
	{entete: exportEnteteNomComplet, requise: true},
	{entete: enteteTelephoneImport, requise: true},
	{entete: exportEnteteDepartement, requise: true},
	{entete: exportEnteteIef},
	{entete: exportEnteteNotes},
	{entete: exportEnteteEtablissement},
...
var (
	relationsImport = tableLibellesImport(map[string]string{
		exportLibelleNonQualifie: string(db.RepresentantRelationINCONNU), "Contacté": string(db.RepresentantRelationCONTACTE),
		exportLibelleAccepte: string(db.RepresentantRelationAMBASSADEUR), "Refusé": string(db.RepresentantRelationREFUS),
	})
	whatsappImport = tableLibellesImport(map[string]string{
		exportLibelleNonDemande: string(db.WhatsappStatusNONDEMANDE), exportLibelleMemeNumero: string(db.WhatsappStatusMEMENUMERO),
		exportLibelleAutreNumero: string(db.WhatsappStatusAUTRENUMERO), libelleAucunImport: string(db.WhatsappStatusAUCUN),
	})
	issuesImport = tableLibellesImport(map[string]string{
		exportLibelleJoint: issueJointImport, exportLibelleInjoignable: string(db.CallOutcomeUNREACHABLE), exportLibelleRefus: exportCleRefus,
		exportLibelleFauxNumero: exportCleMauvaisNumero, libelleAutreImport: string(db.CallOutcomeOTHER),
	})
...
func lireRepresentantImport(cellules map[string]string, numero int, brut any) (any, *erreurLigneImport) {
	...
	telephone, err := normaliserTelephone(cellules[enteteRepresentantImport(1)], etat.region)
	if err != nil {
...
var colonnesProspectsImport = []colonneImport{
	{entete: exportEnteteNom, requise: true},
	{entete: exportEntetePrenom, requise: true},
	{entete: enteteTelephoneImport, requise: true},
	{entete: "Téléphone du représentant", requise: true},
	{entete: libelleBanqueImport, requise: true},
	{entete: exportEnteteSyndicat, requise: true},
	{entete: exportEnteteMethodeEnrolement},
}
...
var (
	...
	libellesEnrolementImport = []string{"RDV CPI", "Plateforme en ligne", "Mail", exportEnteteWhatsapp}
)
...
func lireProspectImport(cellules map[string]string, numero int, brut any) (any, *erreurLigneImport) {
	...
	telephone, err := normaliserTelephone(brutTelephone, etat.region)
	if err != nil {
	...
	telephoneRepresentant, err := normaliserTelephone(brutRepresentant, etat.region)
	if err != nil {
...
var colonnesGrandPublicImport = []colonneImport{
	{entete: exportEntetePrenom, alias: []string{"Prénoms"}},
	{entete: exportEnteteNom, alias: []string{"Nom de famille", "Noms"}, requise: true},
	{entete: enteteTelephoneImport, alias: []string{"Tel", "Numéro", "Numéro de téléphone", "Contact"}, requise: true},
	{entete: exportEnteteProfession, alias: []string{"Métier", "Activité"}},
	{entete: exportEnteteSyndicat},
	...
	{entete: exportEnteteCanalProvenance, alias: []string{"Canal", "Provenance", "Source"}},
	{entete: formulaireLibelleEmployeur, alias: []string{"Ministère", "Entreprise", "Employeur / Entreprise"}},
	{entete: exportEnteteTypeContrat, alias: []string{"Contrat"}},
	{entete: exportEnteteAnciennete, alias: []string{"Ancienneté", "Ancienneté chez l’employeur"}},
	{entete: exportEnteteLieuActivite, alias: []string{"Lieu de travail", "Marché"}},
	{entete: exportEnteteModeEpargne, alias: []string{"Épargne"}},
	{entete: exportEntetePaysResidence, alias: []string{"Pays", "Résidence"}},
	{entete: exportEnteteVilleResidence, alias: []string{"Ville"}},
...
var (
	...
	modesEpargneImport = tableLibellesImport(map[string]string{
		"Tontine": "TONTINE", exportLibelleMobileMoney: modeMobileMoneyImport, "Mobile Money": modeMobileMoneyImport,
		"Orange Money": modeMobileMoneyImport, "Wave": modeMobileMoneyImport,
		libelleBanqueImport: "BANQUE", libelleAucunImport: exportCleAucun,
	})
