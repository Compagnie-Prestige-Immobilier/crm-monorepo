...
var campagnesEcriture = []Role{Admin, Superviseur}

var gardeCampagnes = map[string][]Role{
	"POST /api/v1/lots-export":                       campagnesEcriture,
	"POST /api/v1/lots-export/apercu":                campagnesEcriture,
	"GET /api/v1/lots-export":                        encadrement,
	"GET /api/v1/lots-export/{id}":                   encadrement,
	"PATCH /api/v1/lots-export/{id}":                 campagnesEcriture,
	"DELETE /api/v1/lots-export/{id}":                admin,
	"GET /api/v1/lots-export/{id}/fiches":            encadrement,
	"POST /api/v1/lots-export/{id}/reaffectation":    campagnesEcriture,
	"POST /api/v1/lots-export/{id}/retrait":          campagnesEcriture,
	"GET /api/v1/lots-export/{id}/export.xlsx":       encadrement,
	"GET /api/v1/lots-export/{id}/programme.pdf":     encadrement,
	"GET /api/v1/lots-export/{id}/fiches-recues.pdf": encadrement,
	"GET /api/v1/lots-export/{id}/programmes.zip":    encadrement,
}

var lotLibellesIssueAppel = map[string]string{
	exportCleMethodeObtenue:           exportLibelleMethodeObtenue,
	string(db.CallOutcomeUNREACHABLE): exportLibelleInjoignable,
	lotIssueCallback:                  exportLibelleARappeler,
	exportCleRefus:                    exportLibelleRefus,
	exportCleMauvaisNumero:            exportLibelleMauvaisNumero,
...
type CampagneObjectif struct {
	TeleconseillerID string `json:"teleconseillerId" format:"uuid"`
...
func (s *service) lotEquipe(ctx context.Context, ids []string) ([]lotTeleconseiller, error) {
	...
	if len(uniques) != len(ids) || len(rows) != len(uniques) {
		return nil, problem(http.StatusUnprocessableEntity, "LOT_EXPORT_TELECONSEILLER_INVALIDE",
...
func (s *service) campagneCreer(ctx context.Context, in *CampagneCreationInput) (*CampagneOutput, error) {
	...
	if lotSurRepresentants(in.Body.Cible) == (in.Body.Representants == nil) {
		return nil, problem(http.StatusUnprocessableEntity, "LOT_EXPORT_FILTRES_REQUIS",
...
func (s *service) lotEcrireCampagne(ctx context.Context, createurID string, in *CampagneCreationInput,
	...
	if len(fiches) == 0 {
		return "", problem(http.StatusUnprocessableEntity, "LOT_EXPORT_CIBLE_VIDE",
...
func (s *service) lot(ctx context.Context, id string) (*db.LotParIdRow, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, problem(http.StatusNotFound, "LOT_EXPORT_NOT_FOUND", campagneIntrouvable)
	}
	if err != nil {
		return nil, err
	}
...
type CampagnesListeInput struct {
	...
	CreatedById string `query:"createdById" format:"uuid"`
...
type CampagneReaffecterInput struct {
	...
	Body struct {
		...
		VersTeleconseillerID string `json:"versTeleconseillerId" format:"uuid"`
	}
}
...
func (s *service) campagneReaffecter(ctx context.Context, in *CampagneReaffecterInput) (*CampagneDetailOutput, error) {
	...
	if len(deplacables) == 0 {
		return nil, problem(http.StatusUnprocessableEntity, "LOT_EXPORT_REAFFECTATION_VIDE",
...
func (s *service) campagneRetrait(ctx context.Context, in *CampagneRetraitInput) (*CampagneDetailOutput, error) {
	...
	if len(restants) == 0 {
		return nil, problem(http.StatusUnprocessableEntity, "LOT_EXPORT_EQUIPE_VIDE",
...
func (s *service) lotAppliquerMouvements(ctx context.Context, row *db.LotParIdRow, lotMouvements []lotMouvement,
	...
	defer func() { _ = tx.Rollback(ctx) }()
	q := s.q.WithTx(tx)
	auteur := utilisateurCourant(ctx).ID
	fiches := 0
	...
	if err := auditer(ctx, q, auteur, action, "lot_export", row.ID, nil, details); err != nil {
		return err
	}
...
func monterCampagnes(api huma.API, s *service) {
