...
func referentielsConstruire() (map[string]*referentielsListe, []referentielsChamp) {
	...
	rolesVisite := []Role{Admin, Direction}
	...
	cDepartementID := referentielsChamp{
		nom: representantChampDep, sel: `t."departementId"`,
		vers: func(i *ReferentielsItem) any { return &i.DepartementID },
	}
	cDepartementName := referentielsChamp{
		nom: "departementName", sel: colonneDepartementNom,
	...
	listes := map[string]*referentielsListe{
		nomBanques: {
			...
			rang: referentielsTriSortNom, lecture: tous, ecriture: encadrement,
		},
		nomSyndicats: {
			champs: []referentielsChamp{
		...
		"canaux-provenance": {
			...
			rang: referentielsTriPosition, lecture: tous, ecriture: encadrement,
		},
		nomProfessions: {
			champs: []referentielsChamp{
		...
		nomPays: {
			...
			rang:   referentielsTriPosition, lecture: tous,
		},
		"income-bands": {
			champs: []referentielsChamp{
			...
			rang: `t."position", t."minXof"`, lecture: tous, ecriture: encadrement,
		},
		nomOffers: {
			champs: []referentielsChamp{
				referentielsRegle(cCode, 1, 40, true), referentielsRegle(cLabel, 2, 120, true),
		...
		"bank-rejection-reasons": {
			...
			rang:   referentielsTriSortLabel, lecture: banque,
		},
		...
		nomRegions: {
			...
			rang:   referentielsSelName, lecture: tous,
		},
		nomDepartements: {
			...
			rang: referentielsSelName, lecture: tous, ecriture: encadrement,
		},
		nomIefs: {
			...
			rang: `d."name", t."name"`, filtre: "departementId", lecture: tous,
		},
	}
...
func (l *referentielsListe) introuvableErreur() error {
	...
	return problem(http.StatusNotFound, code, "Entrée de référentiel introuvable.")
}
...
func (l *referentielsListe) erreurEcriture(err error) error {
	...
	p := problem(http.StatusConflict, code, "Cette valeur existe déjà dans cette liste.")
...
func referentielsAutorisee(kind string, role Role, ecriture bool) (*referentielsListe, error) {
	l, connue := referentielsListes[kind]
	if !connue {
		return nil, problem(http.StatusNotFound, "NOT_FOUND", "Référentiel inconnu.")
	}
	roles := l.lecture
	if ecriture {
		roles = l.ecriture
	...
	if !autorise(roles, role) {
		return nil, problem(http.StatusForbidden, "FORBIDDEN", "Accès refusé.")
	}
	return l, nil
}
...
func (s *service) referentielsGet(ctx context.Context, in *ReferentielsListeInput) (*ReferentielsListeOutput, error) {
	l, err := referentielsAutorisee(in.Kind, utilisateurCourant(ctx).Role, false)
	if err != nil {
		return nil, err
	}
...
func (s *service) referentielsCreer(ctx context.Context, in *ReferentielsCreerInput) (*ReferentielsItemOutput, error) {
	l, err := referentielsAutorisee(in.Kind, utilisateurCourant(ctx).Role, true)
	if err != nil {
		return nil, err
	}
...
func (s *service) referentielsSystemeFige(ctx context.Context, l *referentielsListe, id string, e *ReferentielsEntree) error {
	...
	return problem(http.StatusConflict, referentielsVisiteSysteme,
...
var (
	referentielsEffetsJoints = []string{issueJointImport, "REFUSED", "SCHEDULE_CALLBACK", "WRONG_NUMBER"}
...
type ReferentielsCreerStatutInput struct {
	Body struct {
		...
		RetryAfterMinutes *int32  `json:"retryAfterMinutes,omitempty" minimum:"5" maximum:"10080"`
...
func referentielsRappelAutorise(effet, code string, requiresCallback bool) error {
	...
	return problem(http.StatusConflict, code,
...
func referentielsLibreOuConflit(id, code, message string, err error) error {
	...
	p := problem(http.StatusConflict, code, message)
...
func (s *service) referentielsStatutCreer(ctx context.Context, in *ReferentielsCreerStatutInput) (*ReferentielsStatutOutput, error) {
	...
	if !referentielsCodeValide.MatchString(code) {
		return nil, problem(http.StatusBadRequest, "STATUT_QUALIFICATION_LABEL_UNUSABLE",
	...
	row, err := s.q.InsertStatutQualification(ctx, db.InsertStatutQualificationParams{
		...
		RetryAfterMinutes: in.Body.RetryAfterMinutes,
...
func (s *service) referentielsStatutParID(ctx context.Context, id string) (db.StatutsQualification, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return row, problem(http.StatusNotFound, "STATUT_QUALIFICATION_NOT_FOUND", "Statut de qualification introuvable.")
	}
	return row, err
}
...
func referentielsRegleStatutIntacte(existant *db.StatutsQualification, in *ReferentielsModifierStatutInput) error {
	if existant.IsSystem && (in.Body.RequiresCallback != nil || in.Body.RequiresComment != nil) {
		return problem(http.StatusConflict, "STATUT_QUALIFICATION_SYSTEM_IMMUTABLE",
...
func (s *service) referentielsStatutModifier(ctx context.Context, in *ReferentielsModifierStatutInput) (*ReferentielsStatutOutput, error) {
	...
	row, err := s.q.UpdateStatutQualification(ctx, db.UpdateStatutQualificationParams{
		...
		RetryAfterMinutes: in.Body.RetryAfterMinutes, RelationStatus: relation,
	})
	if err != nil {
		return nil, err
	}
...
func (s *service) referentielsStatutActiver(ctx context.Context, in *ReferentielsActiverInput) (*ReferentielsStatutOutput, error) {
	...
	if !in.Body.IsActive && existant.IsActive {
		...
		if restants == 0 {
			return nil, problem(http.StatusConflict, "STATUT_QUALIFICATION_LAST_OF_BRANCH",
...
func (s *service) referentielsMotifParID(ctx context.Context, id string) (db.CallOutcomeReason, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return row, problem(http.StatusNotFound, "OUTCOME_REASON_NOT_FOUND", "Motif d'issue introuvable.")
	}
	return row, err
}
...
func referentielsRegleMotifIntacte(existant *db.CallOutcomeReason, in *ReferentielsModifierMotifInput) error {
	...
	if existant.IsSystem && regleTouchee {
		return problem(http.StatusConflict, "OUTCOME_REASON_SYSTEM_IMMUTABLE",
...
func (s *service) referentielsMotifActiver(ctx context.Context, in *ReferentielsActiverInput) (*ReferentielsMotifOutput, error) {
	...
	if existant.IsSystem {
		return nil, problem(http.StatusConflict, "OUTCOME_REASON_SYSTEM_IMMUTABLE",
...
var gardeReferentiels = map[string][]Role{
	"GET /api/v1/referentiels":                         tous,
	"GET /api/v1/referentiels/{kind}":                  tous,
	"POST /api/v1/referentiels/{kind}":                 encadrement,
	"PATCH /api/v1/referentiels/{kind}/{id}":           encadrement,
	"GET /api/v1/statuts-qualification":                tous,
	"GET /api/v1/statuts-qualification/administration": encadrement,
	"POST /api/v1/statuts-qualification":               encadrement,
	"PATCH /api/v1/statuts-qualification/{id}":         encadrement,
	"POST /api/v1/statuts-qualification/{id}/active":   encadrement,
	"GET /api/v1/call-outcome-reasons":                 tous,
	"GET /api/v1/call-outcome-reasons/administration":  encadrement,
	"POST /api/v1/call-outcome-reasons":                encadrement,
	"PATCH /api/v1/call-outcome-reasons/{id}":          encadrement,
	"POST /api/v1/call-outcome-reasons/{id}/active":    encadrement,
}

func monterReferentiels(api huma.API, s *service) {
