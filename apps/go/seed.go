...
// Même forme que packages/database/src/seed.ts : 20 upsert de référentiels dans
// une transaction, contrôle bloquant sur le nombre de départements, admin et
// fixtures créés seulement s'ils sont absents.
func semer(ctx context.Context, pool *pgxpool.Pool, _ *config) error {
...
func seedSemerAdmin(ctx context.Context, q *db.Queries) error {
	autoriseDefaut := env("NODE_ENV", "") == "development"
	...
	fullName := env("SEED_ADMIN_FULL_NAME", "Administrateur CPI")
	...
	condensat, err := hacherMotDePasse(password)
	if err != nil {
		return err
	}
	id, err := seedNouvelID()
...
func seedSemerFixtures(ctx context.Context, q *db.Queries) error {
	...
	password := env("SEED_FIXTURE_PASSWORD", "ChangeMoi123456")
	if len(password) < 12 {
