...
/** L'agent d'accueil ne porte que ce qu'il peut lire de sa journée. */
...
export function catalogueVisites(role: Role): Readonly<Record<string, SourceVisite>> {
  if (role !== 'ACCUEIL') return SOURCES_VISITES;
  return Object.fromEntries(
    SOURCES_ACCUEIL.flatMap((source) => {
      const entree = SOURCES_VISITES[source];
      return entree === undefined ? [] : [[source, entree] as const];
    }),
  );
}
