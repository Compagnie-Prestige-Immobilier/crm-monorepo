...
export type WhatsappStatus = NonNullable<Schemas['FormulaireCorps']['whatsappStatus']>;
...
const SECTIONS: readonly Section[] = [
  {
    ...
    widgets: {
      ...
      email: { saisie: 'texte', type: 'email', autoComplete: 'email', longueurMax: 160 },
    },
  },
  {
    ...
    widgets: {
      ...
      dureeEtablissementMois: { saisie: 'liste', source: 'dureesEtablissement', nombre: true },
...
function formatInvalide(widget: Widget, valeur: string): string | undefined {
  ...
  if (widget.type === 'tel' && valeur.replaceAll(/\D/gu, '').length < 6) return 'Numéro incomplet.';
  return undefined;
}
