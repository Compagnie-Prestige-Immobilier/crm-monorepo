...
var gardeQualification = map[string][]Role{
	"POST /api/v1/rep-campaigns/attempts":                parcours,
	"POST /api/v1/phase2/call-attempts":                  parcours,
	"GET /api/v1/phase2/callbacks":                       parcours,
	"POST /api/v1/phase2/callbacks/{id}/cancel":          {Admin, Commercial, ChargeClientele},
	"POST /api/v1/ouvertures":                            parcours,
	"GET /api/v1/ouvertures/courante":                    parcours,
	"PUT /api/v1/ouvertures/{id}/brouillon":              parcours,
	"GET /api/v1/ouvertures/ouvertes":                    {Admin, Superviseur},
	"POST /api/v1/ouvertures/{id}/liberation":            {Admin, Superviseur},
	"GET /api/v1/ouvertures/comptage":                    parcours,
	"GET /api/v1/suggestions":                            parcours,
	"PATCH /api/v1/suggestions/{id}":                     parcours,
	"POST /api/v1/phase2/call-attempts/{id}/note-vocale": parcours,
	"GET /api/v1/phase2/call-attempts/{id}/note-vocale":  parcours,
	"POST /api/v1/presence/beat":                         tous,
}
...
func monterQualification(api huma.API, s *service) {
...
func qualificationVoitTout(role Role) bool {
	return role == Admin || role == Superviseur || role == Direction
}
...
var qualificationOutcomeParEffet = map[db.StatutQualificationEffect]string{
	db.StatutQualificationEffectREACHED:          issueJointImport,
...
func qualificationReglesRepIssue(b *QualificationRepAttemptBody, comment *string) error {
	if b.Outcome == string(db.CallOutcomeOTHER) && comment == nil {
		return problem(http.StatusBadRequest, "REP_CAMPAIGN_COMMENT_REQUIRED",
			"L’issue « Autre » exige un commentaire : sans lui, la case ne dit rien.")
	}
	if b.PromisedProspects != nil && b.Outcome != "PROSPECTS_PROMISED" {
		return problem(http.StatusBadRequest, "REP_CAMPAIGN_PROMISED_NOT_ALLOWED",
	...
	if b.Outcome == qualificationCallback && b.CallbackAt == nil {
		return problem(http.StatusBadRequest, "REP_CAMPAIGN_CALLBACK_AT_REQUIRED",
...
func qualificationWhatsappRepresentant(b *QualificationRepAttemptBody, rep *db.RepresentantAQualifierRow, region string) (qualificationPatchWhatsapp, error) {
	...
	if b.WhatsappE164 != nil {
		e164, err := normaliserTelephone(*b.WhatsappE164, region)
		if err != nil {
			return p, err
		}
		numero = &e164
	...
	if numero != nil && statut != qualificationAutreNumero {
		return p, problem(http.StatusBadRequest, "WHATSAPP_NUMBER_NOT_ALLOWED",
	...
	if statut == qualificationAutreNumero {
		if numero == nil && rep.WhatsappE164 == nil {
			return p, problem(http.StatusBadRequest, "WHATSAPP_NUMBER_REQUIRED",
...
func qualificationNumeroCorrige(b *QualificationRepAttemptBody, rep *db.RepresentantAQualifierRow, region string) (string, error) {
	...
	e164, err := normaliserTelephone(*b.Phone, region)
	if err != nil {
		return "", err
	}
...
func (s *service) qualificationStatutCoherent(ctx context.Context, b *QualificationRepAttemptBody, comment *string) (db.StatutQualificationParIdRow, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return statut, problem(http.StatusBadRequest, "REP_STATUT_QUALIFICATION_UNKNOWN",
	...
	if !statut.IsActive {
		return statut, problem(http.StatusBadRequest, "REP_STATUT_QUALIFICATION_INACTIVE",
	...
	if b.Outcome != attendue {
		return statut, problem(http.StatusBadRequest, "REP_OUTCOME_STATUT_MISMATCH",
	...
	if qualificationContreditRattachement(b.RelationStatus, statut.RelationStatus) {
		return statut, problem(http.StatusBadRequest, "REP_RELATION_STATUT_MISMATCH",
			"Le statut « "+statut.Label+" » contredit la réponse au rattachement comme représentant.")
	}
	if statut.RequiresComment && comment == nil {
		return statut, problem(http.StatusBadRequest, "REP_STATUT_MOTIF_REQUIRED",
...
func (s *service) qualificationRepAbsent(ctx context.Context, id string) error {
	...
	if ailleurs {
		return problem(http.StatusForbidden, "REP_CAMPAIGN_NOT_ASSIGNED", "Ce représentant n’est pas dans vos campagnes.")
	}
	return problem(http.StatusNotFound, "REPRESENTANT_NOT_FOUND", "Représentant introuvable.")
}

func (s *service) qualificationPreparerAppelRep(ctx context.Context, u *Utilisateur, b *QualificationRepAttemptBody, appel *qualificationAppelRep) error {
	var err error
...
func (*service) qualificationNumeroDisponible(ctx context.Context, q *db.Queries, a *qualificationAppelRep) error {
	...
	return problem(http.StatusConflict, "REPRESENTANT_PHONE_CONFLICT",
		"Ce numéro est déjà celui d’un représentant enregistré par "+nom+".")
}

var qualificationChampsFiche = []string{
	representantChampNom, prospectChampPrenom, representantChampTel, prospectChampEtablissement, representantChampNotes, representantChampDep,
	representantChampIef, prospectChampWhatsappStatut, prospectChampWhatsappNumero, prospectChampProfession, representantChampSyndicat, representantChampConnaitUES, representantChampContacte,
}
...
func (*service) qualificationMajRepresentant(ctx context.Context, q *db.Queries, a *qualificationAppelRep) error {
	...
	return auditer(ctx, q, a.u.ID, "representant.fiche.APPEL", "representant", a.b.RepresentantID, deA, deP)
}
...
func (*service) qualificationBasculerRelation(ctx context.Context, q *db.Queries, a *qualificationAppelRep) error {
	...
	if !qualificationTransitionLegale(qualificationTransitionsRelation, depuis, *a.relation) {
		return problem(http.StatusForbidden, "REPRESENTANT_RELATION_TRANSITION_REFUSED",
...
var qualificationReglesEffet = map[db.CallOutcomeEffect]qualificationRegleIssue{
	db.CallOutcomeEffectCLOSEMETHOD:      {clot: true, phase2Status: exportCleMethodeObtenue, exigeMethode: true},
...
var qualificationMotifsSysteme = map[string]qualificationMotifIssue{
	exportCleMethodeObtenue:           {label: exportLibelleMethodeObtenue, effet: db.CallOutcomeEffectCLOSEMETHOD},
	qualificationCallback:             {label: exportLibelleARappeler, effet: db.CallOutcomeEffectSCHEDULECALLBACK},
	string(db.CallOutcomeUNREACHABLE): {label: exportLibelleInjoignable, effet: db.CallOutcomeEffectKEEPOPEN},
	qualificationRefusee:              {label: exportLibelleRefus, effet: db.CallOutcomeEffectCLOSEREFUSED},
	qualificationFauxNumero:           {label: exportLibelleFauxNumero, effet: db.CallOutcomeEffectCLOSEWRONGNUMBER},
	string(db.CallOutcomeOTHER):       {label: exportLibelleAutre, effet: db.CallOutcomeEffectKEEPOPEN, exigeCommentaire: true},
}
...
func (s *service) qualificationMotifDeLIssue(ctx context.Context, b *QualificationCallAttemptBody) (qualificationMotifIssue, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		...
		return systeme, problem(http.StatusBadRequest, "PHASE2_REASON_UNKNOWN", "Motif d’issue inconnu : "+code+".")
	}
	if err != nil {
		return systeme, err
	}
	if demande && !row.IsActive {
		return systeme, problem(http.StatusBadRequest, "PHASE2_REASON_INACTIVE",
			"Le motif « "+row.Label+" » a été retiré du référentiel.")
	}
	if demande && row.Effect != systeme.effet {
		return systeme, problem(http.StatusBadRequest, "PHASE2_REASON_OUTCOME_MISMATCH",
...
func qualificationNormaliserTentative(b *QualificationCallAttemptBody, motif *qualificationMotifIssue) (qualificationTentative, error) {
	...
	if t.regle.exigeMethode && b.Method == nil {
		return t, problem(http.StatusBadRequest, "PHASE2_METHOD_REQUIRED",
			"Une méthode d’enrôlement est obligatoire quand la méthode a été obtenue.")
	}
	if !t.regle.exigeMethode && b.Method != nil {
		return t, problem(http.StatusBadRequest, "PHASE2_METHOD_NOT_ALLOWED",
	...
	if motif.exigeCommentaire && t.comment == nil {
		return t, problem(http.StatusBadRequest, "PHASE2_COMMENT_REQUIRED",
...
func qualificationRappelPromis(b *QualificationCallAttemptBody, motif *qualificationMotifIssue, regle *qualificationRegleIssue) (time.Time, error) {
	...
	if b.CallbackAt == nil {
		if motif.exigeRappel {
			return absent, problem(http.StatusBadRequest, "PHASE2_CALLBACK_AT_REQUIRED",
				"L’issue « "+motif.label+" » exige la date du rappel promis.")
		}
		return absent, nil
	}
	if !regle.accepteCallbackAt {
		return absent, problem(http.StatusBadRequest, "PHASE2_CALLBACK_AT_NOT_ALLOWED",
	...
	if quand.Before(b.ClientCreatedAt.Add(-qualificationToleranceHorloge)) {
		return absent, problem(http.StatusBadRequest, "PHASE2_CALLBACK_AT_PAST",
...
func qualificationCourrielRecueilli(b *QualificationCallAttemptBody) (string, error) {
	...
	if !qualificationCourriel.MatchString(*brut) {
		return "", problem(http.StatusBadRequest, "PHASE2_EMAIL_INVALID",
...
func qualificationRendezVousFixe(b *QualificationCallAttemptBody) (time.Time, error) {
	...
	if b.RendezVousAt == nil {
		if pris {
			return absent, problem(http.StatusBadRequest, "PHASE2_RENDEZ_VOUS_REQUIRED",
				"Le RDV CPI exige la date et l’heure du rendez-vous.")
		}
		return absent, nil
	}
	if !pris {
		return absent, problem(http.StatusBadRequest, "PHASE2_RENDEZ_VOUS_NOT_ALLOWED",
			"Une date de rendez-vous n’est admise que pour la méthode « RDV CPI ».")
	}
	quand := b.RendezVousAt.UTC()
	if quand.Before(b.ClientCreatedAt.Add(-qualificationToleranceHorloge)) {
		return absent, problem(http.StatusBadRequest, "PHASE2_RENDEZ_VOUS_PAST",
...
func qualificationConversionChues(b *QualificationCallAttemptBody, projet db.Projet, revenu *string) error {
	...
	if *b.Method == "PHYSICAL" {
		return problem(http.StatusBadRequest, "PHASE2_METHOD_RETIREE",
			"La méthode « Physique » est remplacée par « RDV CPI », qui exige la date du rendez-vous.")
	}
	if revenu == nil {
		return problem(http.StatusBadRequest, "PHASE2_REVENU_REQUIRED",
			"La conversion CHUES exige la tranche de revenu mensuel du prospect.")
	}
	if b.DureeEtablissementMois == nil {
		return problem(http.StatusBadRequest, "PHASE2_DUREE_FONCTION_REQUIRED",
...
func qualificationDejaTraite(etat *QualificationProspectPhase2StateDTO) error {
	p := problem(http.StatusConflict, "PHASE2_ALREADY_COMPLETED",
...
func (s *service) qualificationProspectAttribue(ctx context.Context, u *Utilisateur, prospectID string) error {
	...
	if !mien {
		return problem(http.StatusForbidden, "PHASE2_NOT_ASSIGNED", "Ce prospect n’est pas dans vos campagnes.")
	}
	return nil
}
...
func qualificationPreVol(ctx context.Context, q *db.Queries, b *QualificationCallAttemptBody) (prospect db.ProspectPourTentativeRow, parcours db.ParcoursDuProspectRow, duplicate bool, err error) {
	if prospect, err = q.ProspectPourTentative(ctx, b.ProspectID); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			err = problem(http.StatusNotFound, "PHASE2_PROSPECT_NOT_FOUND", "Prospect introuvable ou supprimé.")
		}
	...
	if b.ExpectedRev != nil && *b.ExpectedRev != prospect.Rev {
		return prospect, parcours, false, problem(http.StatusConflict, "REV_CONFLICT",
...
func (s *service) qualificationAppliquerTentative(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody, t *qualificationTentative) (string, QualificationProspectPhase2StateDTO, error) {
...
func qualificationInsererTentative(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody, t *qualificationTentative) error {
	...
	if err == nil && n == 0 {
		return problem(http.StatusConflict, "PHASE2_ATTEMPT_RACE", "Cette tentative vient d’être enregistrée. Réessayez.")
	}
	return err
}
...
func (s *service) qualificationCorrigerProspect(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody, courant *db.ProspectPourTentativeRow) (db.CorrigerProspectParTentativeRow, error) {
...
func qualificationWhatsappProspect(b *QualificationCallAttemptBody, courant *db.ProspectPourTentativeRow, region string) (statut *string, majNumero bool, numero *string, err error) {
	...
	var saisi *string
	if b.WhatsappE164 != nil {
		e164, erreur := normaliserTelephone(*b.WhatsappE164, region)
		if erreur != nil {
...
func qualificationPlanifierRappel(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody, t *qualificationTentative) error {
	if t.callbackAt == nil {
		return nil
	}
...
func qualificationFermerOuvertureProspect(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody) error {
...
func qualificationCloturerParcours(ctx context.Context, q *db.Queries, u *Utilisateur, b *QualificationCallAttemptBody, t *qualificationTentative, prospect *db.CorrigerProspectParTentativeRow, parcours *db.ParcoursDuProspectRow) (string, QualificationProspectPhase2StateDTO, error) {
	var vide QualificationProspectPhase2StateDTO
	at := time.Now().UTC()
