...
var (
	formulaireLimiteLecture  = nouveauLimiteur(30)
	formulaireLimiteEcriture = nouveauLimiteur(5)
)
...
var formulaireChampsPublics = []string{
	prospectChampNom, prospectChampPrenom, prospectChampPhone, prospectChampEmail,
	prospectChampProfession, prospectChampDureeEtablissement, prospectChampFonctionnaire,
	prospectChampType, prospectChampSyndicat, prospectChampBanque, prospectChampEngagement,
	prospectChampRevenu, prospectChampPaiement, prospectChampEtablissement,
	prospectChampDureeSysteme, prospectChampWhatsappStatut, prospectChampWhatsappNumero,
}
...
func formulaireTropDeDemandes() error {
	return problem(http.StatusTooManyRequests, "RATE_LIMITED", "Trop de tentatives. Réessayez dans une minute.")
}
...
type FormulaireDemandeInput struct {
	Jeton string `path:"jeton" format:"uuid"`
...
func (s *service) formulaireRecevoir(ctx context.Context, in *FormulaireDemandeInput) (*ProspectOkOutput, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, problem(http.StatusNotFound, "LIEN_INVALIDE", formulaireLienMort)
	}
	if err != nil {
		return nil, err
	}
...
func (s *service) formulairePrevenir(ctx context.Context, agentID, prospectID string, in *FormulaireDemandeInput, saisie *formulaireSaisie) error {
	...
	if _, err := s.composerNotification(ctx, agentID, &CreationNotification{
...
func (s *service) formulaireAviserParEmail(ctx context.Context, parametres *ProspectParametresChues, superviseurs []db.SuperviseursActifsRow, prospectID, prenomNom, titre, corps, informations string, saisie *formulaireSaisie) error {
	var copies []destinataireBrevo
	for _, membre := range superviseurs {
		if membre.Email != "" {
			copies = append(copies, destinataireBrevo{Email: membre.Email, Nom: membre.FullName})
		}
	}
	for _, adresse := range parametres.DestinatairesSupervision {
		if propre := strings.TrimSpace(adresse); propre != "" {
			copies = append(copies, destinataireBrevo{Email: propre})
		}
	}
	var messages []messageBrevo
	if len(copies) > 0 {
		messages = append(messages, messageBrevo{
	...
	if saisie.email != nil {
		...
		messages = append(messages, messageBrevo{
			Destinataires: []destinataireBrevo{{Email: *saisie.email, Nom: prenomNom}},
	...
	if envoi := transport.envoyer(ctx, messages); envoi.statut != brevoEnvoye {
...
func formulaireResumer(in *FormulaireDemandeInput, saisie *formulaireSaisie) string {
	...
	for _, entree := range []struct {
		...
		{exportEnteteProfession, saisie.profession},
		{exportEnteteEtablissement, saisie.etablissement},
		{formulaireLibelleEmployeur, saisie.employeur},
	} {
...
func (s *service) formulaireRetenir(ctx context.Context, in *FormulaireDemandeInput) (formulaireSaisie, error) {
	...
	if saisie.phoneE164, err = normaliserTelephone(corps.Phone, s.cfg.PhoneRegion); err != nil {
		return saisie, err
	}
	saisie.nom, saisie.prenom = strings.TrimSpace(corps.Nom), strings.TrimSpace(corps.Prenom)
	numero := formulaireGarde(visibles, prospectChampWhatsappNumero, corps.WhatsappE164)
...
func formulaireRetenirChamps(visibles map[string]bool, corps *formulaireCorps, saisie *formulaireSaisie) {
	...
	saisie.etablissement = prospectRogner(formulaireGarde(visibles, prospectChampEtablissement, corps.Etablissement))
	...
	saisie.whatsappStatus = formulaireGarde(visibles, prospectChampWhatsappStatut, corps.WhatsappStatus)
...
func (s *service) formulaireProfession(ctx context.Context, visibles map[string]bool, in *FormulaireDemandeInput, saisie *formulaireSaisie) error {
	...
	saisie.profession = prospectRogner(formulaireGarde(visibles, prospectChampProfession, in.Body.Profession))
	return nil
}
...
func formulaireValeurSaisie(corps *formulaireCorps, champ string) bool {
	presents := map[string]bool{
		...
		prospectChampPrenom: strings.TrimSpace(corps.Prenom) != "",
		prospectChampPhone:  corps.Phone != "", prospectChampEmail: corps.Email != nil,
		prospectChampProfession:         corps.ProfessionID != nil || corps.Profession != nil,
		prospectChampEtablissement:      corps.Etablissement != nil,
		...
		prospectChampWhatsappStatut:     corps.WhatsappStatus != nil,
		prospectChampWhatsappNumero:     corps.WhatsappE164 != nil,
	}
...
func formulaireManquants(reglages *ProspectReglagesConversion, visibles map[string]bool, retenus map[string]string, in *FormulaireDemandeInput) error {
	...
	return problem(http.StatusBadRequest, "CHAMPS_OBLIGATOIRES", "Renseignez "+strings.Join(libelles, ", ")+" avant d’envoyer.")
}
...
func (s *service) formulaireCompleter(ctx context.Context, existant *db.ProspectParTelephoneRow, saisie *formulaireSaisie) error {
	...
	formulaireCombler(maj, prospectChampPrenom, &existant.Prenom, &saisie.prenom)
	formulaireCombler(maj, prospectChampEmail, existant.Email, saisie.email)
	formulaireCombler(maj, prospectChampProfession, existant.Profession, saisie.profession)
	...
	formulaireCombler(maj, prospectChampEtablissement, existant.Etablissement, saisie.etablissement)
	...
	if existant.WhatsappStatus == db.WhatsappStatusNONDEMANDE && existant.WhatsappE164 == nil {
		if statut, numero := prospectWhatsapp(formulaireWhatsapp(saisie), nil, existant.PhoneE164); statut != nil {
			...
			maj.set(prospectChampWhatsappNumero, numero)
		}
	}
...
func formulaireVerifierTurnstile(ctx context.Context, jeton *string) error {
	...
	if config.secret == "" {
		if !config.degradee {
			return problem(http.StatusServiceUnavailable, "CAPTCHA_INDISPONIBLE",
...
func formulaireCaptchaRefuse() error {
	return problem(http.StatusBadRequest, "CAPTCHA_REFUSE",
...
var gardeFormulairePublic = map[string][]Role{
	"GET /api/v1/formulaire-public/formulaire": {Public},
	"POST /api/v1/formulaire-public/{jeton}":   {Public},
}

func monterFormulairePublic(api huma.API, s *service) {
	formulaireTurnstile = formulaireConfigTurnstile{
		secret:   strings.TrimSpace(env("TURNSTILE_SECRET_KEY", "")),
		siteKey:  strings.TrimSpace(env("TURNSTILE_SITE_KEY", "")),
		degradee: env("TURNSTILE_ALLOW_DEGRADED", "false") == prospectVrai,
	}
