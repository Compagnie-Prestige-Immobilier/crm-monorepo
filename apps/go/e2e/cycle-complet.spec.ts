...
test.afterAll(async () => {
  ...
  await ecrire(
    `DELETE FROM representant_call_attempts WHERE "representantId" IN
...
test.describe('parcours 15, du representant importe a l encaissement', () => {
  ...
  test('l administrateur importe le representant et l attribue au teleconseiller', async ({
    ...
    const classeur = await classeurExcel(
      ...
      ['Nom complet', 'Téléphone', 'Département', 'IEF', 'Notes', 'Établissement',
        'Statut relation', 'WhatsApp', 'Chargé de compte'],
      [
        [
          NOM_REPRESENTANT, TELEPHONE_REPRESENTANT, departement.name, '', '', 'Lycée du cycle',
  ...
  test('le teleconseiller qualifie le representant', async ({ browser }) => {
    ...
    await expect(page.getByText(`Appel enregistré pour ${NOM_REPRESENTANT}.`)).toBeVisible();
    await contexte.close();
    ...
    expect(
      await compter(
        `SELECT count(*) AS n FROM representant_call_attempts a
  ...
  test('le teleconseiller ajoute puis convertit le prospect', async ({ browser }) => {
    ...
    await page.getByLabel(/^Numéro/u).fill(numero().slice(4));
    ...
    await page.getByRole('dialog').getByRole('button', { name: 'Ouvrir' }).click();
    await page.getByRole('button', { name: 'Joignable' }).click();

    await page.getByRole('button', { name: /^Revenu mensuel/u }).click();
...
async function repondre(page: Page, question: string, reponse: string): Promise<void> {
  await page.getByRole('group', { name: question }).getByRole('button', { name: reponse }).click();
}
