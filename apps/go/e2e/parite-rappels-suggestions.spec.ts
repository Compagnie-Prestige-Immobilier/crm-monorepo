...
async function promettre(client: Client, sujet: string, heures: number): Promise<Promesse> {
  ...
  await client.query(
    `INSERT INTO call_attempts (id, "prospectId", "performedById", outcome, comment, "clientCreatedAt")
     VALUES ($1, $2, $3, 'CALLBACK', $4, now())`,
    [tentative, fiche.id, REPRENEUR.id, commentaire],
  );
  ...
  await client.query(
    ...
    [
      ...
      fiche.id,
      REPRENEUR.id,
      new Date(Date.now() + heures * HEURE_MS).toISOString(),
      commentaire,
      tentative,
    ],
  );
  await client.query(
    `UPDATE prospects SET "lastCallAt" = now(), "lastCallById" = $2, "lastCallOutcome" = 'CALLBACK'
      WHERE id = $1`,
    [fiche.id, REPRENEUR.id],
  );
 ...
 */
async function semerDemandes(client: Client): Promise<void> {
  const banques = await client.query<{ id: string }>('SELECT id FROM banques ORDER BY name LIMIT 1');
  const banqueId = banques.rows[0]?.id;
  if (banqueId === undefined) throw new Error('aucune banque au referentiel');

  for (const [prenom, demandeur] of [
    ['Sienne', banquier.id],
    ...
    await client.query(
      ...
      [randomUUID(), NOM_DEMANDE, prenom, numeroUnique(), banqueId, demandeur, MOTIF_REFUS],
    );
  }
}
...
async function ouvrirSession(browser: Browser): Promise<void> {
  const contexte = await browser.newContext({
    baseURL: BASE_URL,
    extraHTTPHeaders: { 'X-Forwarded-For': REPRENEUR.adresse },
  });
...
test.describe('parite rappels et contacts, le teleconseiller', () => {
  ...
  test('les trois onglets, le compteur de retards et la file de la semaine', async ({ page }) => {
    ...
    await expect(
      ...
      'une echeance depassee porte son retard, pas « Sans objet »',
    ).toHaveCount(0);
  ...
  test('l’echeance promise a un representant remonte en retard avec son origine', async ({
    page,
  }) => {
    await page.goto(RAPPELS);
    await expect(
      page.getByRole('heading', { name: 'Représentants à reprendre', level: 2 }),
    ).toBeVisible();
  ...
  test('annuler un rappel le retire de l’ecran et de la base', async ({ page }) => {
    await page.goto(RAPPELS);
    await page.getByRole('tab', { name: 'Cette semaine' }).click();

    const cible = fileRappels(page).getByRole('row').filter({ hasText: aAnnuler.commentaire });
    ...
    await expect(
      fileRappels(page).getByRole('row').filter({ hasText: aAnnuler.commentaire }),
  ...
  test('mes contacts listent les prospects et les representants appeles', async ({ page }) => {
    ...
    const prospect = ligne(page, semaine.fiche.nom);
  ...
  test('consigner l’appel ouvre la fiche visee dans la console', async ({ page }) => {
    await page.goto(RAPPELS);
    await page.getByRole('tab', { name: 'Cette semaine' }).click();

    await fileRappels(page)
      .getByRole('row')
...
test.describe('parite rappels et numeros suggeres, l’encadrement', () => {
  ...
  test.fixme(
    'annuler un rappel : le bouton est offert au superviseur, que l’API refuse en 403',
    async ({ page }) => {
      await page.goto(RAPPELS);
      await expect(
        fileRappels(page)
          .getByRole('row')
          .filter({ hasText: depassee.commentaire })
  ...
  test.fixme(
    'solder un numero suggere : l’encadrement, en lecture seule en v1, tranche desormais',
    async ({ page }) => {
      await page.goto(SUGGESTIONS);
...
test.describe('parite rappels et numeros suggeres, une file vide', () => {
  ...
  test('chaque onglet et chaque liste vide disent quoi faire ensuite', async ({ page }) => {
    ...
    await expect(
      page.getByText('Les échéances promises sont tenues.', { exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText('Une échéance se promet en consignant un appel.', { exact: true }),
    ).toBeVisible();
