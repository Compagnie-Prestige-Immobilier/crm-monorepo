...
async function dossier(reference: string): Promise<Dossier> {
...
async function transitions(dossierId: string): Promise<number> {
  return compter(`SELECT count(*) AS n FROM bank_case_transitions WHERE "caseId" = $1`, [dossierId]);
}
...
test.describe('parcours 10, dossiers bancaires', () => {
  ...
  test('ouverture, conflit de revision et encaissement', async ({ page, browser }) => {
    ...
    const avance = await dossier(REFERENCE);
    ...
    expect(await transitions(avance.id)).toBe(2);
    ...
    expect(await transitions(encaisse.id)).toBe(3);
  });
  ...
  test('rejet motive, montant remis a zero', async ({ page }) => {
    ...
    const rejete = await dossier(REFERENCE_REJET);
