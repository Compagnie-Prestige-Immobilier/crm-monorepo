...
test.describe('parité supervision, le volet Activité', () => {
  ...
  test('les faits semés se relisent colonne par colonne, et le tri les réordonne', async ({
    ...
    await expect(
      ...
      'la ligne du superviseur doit relire les quatre tentatives de la journée, la cinquième exclue',
  ...
  test.fixme('exporter l’activité en CSV : le bouton « Exporter en CSV » de la v1 a disparu, le volet Activité n’offre aucun export (v1 CHU-SUP-05)', () => {});
});

test.describe('parité supervision, la présence et les fiches restées ouvertes', () => {
  test.use({ storageState: superviseur.etat });
  ...
  test('un battement de téléconseiller se lit à l’écran et en base', async ({ page }) => {
    ...
    await expect(
      teleconseillers.getByRole('row').filter({ hasText: teleconseiller.identifiant }),
...
test.describe('parité chiffres, l’écran d’un superviseur', () => {
  ...
  test('la période et la plage libre vivent dans l’URL, la plage trop large est refusée', async ({
    page,
  }) => {
    ...
    for (const [libelle, cle] of [
      ...
      const pastille = page.getByRole('button', { name: libelle, exact: true });
  ...
  test('l’écran d’usine pose ses cartes, ses taux relisent les tentatives semées', async ({
    ...
    await tuile(page, JOIGNABILITE, '80,0 %', '4 joints sur 5 fiches');
...
test.describe('parité disposition, composer l’écran des chiffres', () => {
  ...
  test('l’ordre se change au clavier, il survit, et l’autre écran garde le sien', async ({
    ...
    for (const titre of ['Taux de joignabilité des prospects', 'Prospects saisis']) {
      await expect(carte(page, titre), `« ${titre} » d’usine sur Grand Public`).toBeVisible();
    }
  ...
  test('le tiroir ne propose pas une carte déjà posée et ajoute celle qu’on choisit', async ({
    ...
    await expect(
      tiroir.getByRole('heading', { name: ACCEPTATION, exact: true }),
