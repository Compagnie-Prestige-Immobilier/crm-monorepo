...
/**
 * Ligne 1 les en-têtes, ligne 2 l'exemple du modèle que le serveur saute
 * (`imports.go:30`), ligne 3 et suivantes les données. Les colonnes sont lues
 * par POSITION.
 */
...
test.describe('parité CHUES, le registre des représentants', () => {
  ...
  test('le titre de l’onglet nomme l’écran ouvert', async ({ page }) => {
    test.fixme(
      true,
      'la SPA garde `CPI GO` en titre sur tous les écrans : v1 posait « Représentants » par page (index.html:6, aucun titre par route).',
    );
    await page.goto(REGISTRE);
...
test.describe('parité CHUES, l’import en 390 px', () => {
  test.use({
    storageState: compteDe('ADMIN').etat,
  ...
  test('l’écran d’import tient dans le téléphone', async ({ page }) => {
    ...
    expect(
      mesure.largeur,
      `l’écran déborde de ${String(mesure.largeur - mesure.ecran)} px. Coupables : ${mesure.coupables.join(' | ')}`,
