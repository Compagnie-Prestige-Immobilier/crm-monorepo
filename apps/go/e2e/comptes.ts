...
// Une adresse par rôle : le limiteur de connexion compte 10 tentatives par
// minute et par adresse. Préfixe `e2e.` : `fixture.` appartient au seed.
export const COMPTES: Compte[] = ROLES.map((role, rang) => {
  const cle = role.toLowerCase().replaceAll('_', '-');
  return {
    role,
    id: `e2e-${cle}`,
    email: `e2e.${cle}@cpi.sn`,
    identifiant: `e2e.${cle}`,
    nom: `E2E ${role}`,
    adresse: `198.51.100.${String(10 + rang)}`,
    etat: path.join(__dirname, '.auth', PORT, `${cle}.json`),
  };
});
