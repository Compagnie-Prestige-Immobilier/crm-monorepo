...
const (
	...
	lotEnteteCommercial = "Commercial"
...
func (*service) lotProgrammeDe(row *db.LotParIdRow, rows []db.LotItemsPourPdfJourRow, titre, periode string) *lotDonneesProgramme {
	...
	nom := exportEnteteTeleconseiller
...
func (s *service) lotDonneesProgrammeJour(ctx context.Context, id, teleconseillerID string, jour int) (*lotDonneesProgramme, error) {
	...
	if len(rows) == 0 {
		return nil, problem(http.StatusNotFound, "LOT_EXPORT_PROGRAMME_INTROUVABLE",
...
func (s *service) lotDonneesFichesRecues(ctx context.Context, id, teleconseillerID, reaffectationID string) (*lotDonneesProgramme, error) {
	...
	if len(rows) == 0 {
		return nil, problem(http.StatusNotFound, "LOT_EXPORT_FICHES_RECUES_INTROUVABLES",
...
type CampagneProgrammeInput struct {
	...
	TeleconseillerID string `query:"teleconseillerId" required:"true" format:"uuid"`
...
var lotColonnesRepresentants = []struct {
	...
	{exportEnteteTeleconseiller, 26},
	{exportEnteteJour, 8},
	{exportEnteteNomComplet, 30},
	{exportEnteteTelephone, 20},
	{exportEnteteDepartement, 24},
	{exportEnteteIef, 26},
	{lotEnteteCommercial, 26},
	{exportEnteteNotes, 40},
	{exportEnteteSaisiLe, 20},
}
...
var lotColonnesProspects = []struct {
	...
	{exportEnteteJour, 8},
	{exportEnteteNom, 24},
	{exportEntetePrenom, 24},
	{exportEnteteTelephone, 20},
	{lotEnteteBanque, 24},
	{exportEnteteSyndicat, 20},
	{exportEnteteRepresentant, 26},
	{exportEnteteDepartement, 24},
	{lotEnteteCommercial, 26},
	{exportEnteteDateSaisie, 20},
}
...
func (s *service) lotEcrireClasseur(ctx context.Context, row *db.LotParIdRow, sortie huma.Context) error {
	...
	styleEntete, err := classeur.NewStyle(&excelize.Style{
		...
		Fill: excelize.Fill{Type: exportTypeRemplissage, Pattern: 1, Color: []string{"FF630210"}},
	})
	if err != nil {
		return err
	}
