...
var (
	...
	notificationRolesEncadrants = []string{string(Admin), string(Superviseur), string(Direction)}
)
...
func monterNotifications(api huma.API, s *service) {
...
var gardeNotifications = map[string][]Role{
	"POST /api/v1/notifications":                      admin,
	"GET /api/v1/notifications":                       admin,
	"GET /api/v1/notifications/mine":                  tous,
	"GET /api/v1/notifications/audience-preview":      admin,
	"GET /api/v1/notifications/{id}":                  admin,
	"POST /api/v1/notifications/{id}/cancel":          admin,
	"POST /api/v1/notifications/{id}/read":            tous,
	"GET /api/v1/notification-templates":              admin,
	"POST /api/v1/notification-templates":             admin,
	"GET /api/v1/notification-templates/{id}":         admin,
	"PATCH /api/v1/notification-templates/{id}":       admin,
	"POST /api/v1/notification-templates/{id}/render": admin,
}
...
func cronNotifications(nom, defaut string) string {
	parts := notificationMotifHeureCron.FindStringSubmatch(env(nom, defaut))
	if parts == nil {
...
func (s *service) gabaritNotificationVisible(ctx context.Context, id string) (db.NotificationTemplateRow, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return row, problem(http.StatusNotFound, "NOTIFICATION_TEMPLATE_NOT_FOUND", "Gabarit introuvable.")
	}
	return row, err
}
...
func notificationNomDejaPris(err error, nom string) error {
	...
	if errors.As(err, &pg) && pg.Code == "23505" {
		return problem(http.StatusConflict, "NOTIFICATION_TEMPLATE_NAME_CONFLICT",
...
func (s *service) destinatairesNotification(ctx context.Context, audience, role string, ids []string) ([]string, error) {
	...
	case "ROLE":
		if role == "" {
			return nil, problem(http.StatusUnprocessableEntity, "NOTIFICATION_AUDIENCE_ROLE_REQUIRED",
				"Un public « par rôle » exige `audienceRole`.")
		}
	case "DEPARTEMENT":
		return nil, problem(http.StatusUnprocessableEntity, "NOTIFICATION_AUDIENCE_DEPARTEMENT_RETIRED",
	...
	case string(db.NotificationAudienceUSERS):
		if ids = notificationDedoublonner(ids); len(ids) == 0 {
			return nil, problem(http.StatusUnprocessableEntity, "NOTIFICATION_AUDIENCE_USERS_REQUIRED",
...
func (s *service) composerNotification(ctx context.Context, auteur string, in *CreationNotification) (string, error) {
	...
	if in.ScheduledFor != nil && !in.ScheduledFor.After(maintenant) {
		return "", problem(http.StatusUnprocessableEntity, "NOTIFICATION_SCHEDULE_IN_PAST",
	...
	if len(cibles) == 0 {
		return "", problem(http.StatusUnprocessableEntity, "NOTIFICATION_AUDIENCE_EMPTY",
...
func (s *service) creerNotification(ctx context.Context, in *CreerNotificationInput) (*NotificationOutput, error) {
	id, err := s.composerNotification(ctx, utilisateurCourant(ctx).ID, &in.Body)
	if err != nil {
		return nil, err
	}
...
func (s *service) lireNotification(ctx context.Context, id string) (Notification, []NotificationDestinataire, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return Notification{}, nil, problem(http.StatusNotFound, "NOTIFICATION_NOT_FOUND", "Notification introuvable.")
	}
	if err != nil {
...
func (s *service) annulerNotification(ctx context.Context, in *NotificationIDInput) (*NotificationOutput, error) {
	...
	if lignes == 0 {
		...
		return nil, problem(http.StatusConflict, "NOTIFICATION_NOT_SCHEDULED",
...
func (s *service) marquerNotificationLue(ctx context.Context, in *NotificationIDInput) (*NotificationLectureOutput, error) {
	...
	if avancees+horodatees > 0 {
		...
		if total == 0 {
			return nil, problem(http.StatusNotFound, "NOTIFICATION_NOT_FOUND", "Notification introuvable.")
		}
	}
