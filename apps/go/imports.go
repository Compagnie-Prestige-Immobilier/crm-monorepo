...
var (
	limiteurImports           = nouveauLimiteur(5)
...
var gardeImports = map[string][]Role{
	"GET /api/v1/imports":                         admin,
	"POST /api/v1/imports/representants":          admin,
	"POST /api/v1/imports/prospects":              admin,
	"POST /api/v1/imports/prospects-grand-public": admin,
	"POST /api/v1/imports/visites":                admin,
	"GET /api/v1/imports/{id}":                    admin,
	"POST /api/v1/imports/{id}/apply":             admin,
}
...
func reglagesImports() reglagesImport {
	return reglagesImport{
		dir:       env("IMPORTS_DIR", "./storage/imports"),
		...
		balayage:  env("IMPORTS_SWEEP_ENABLED", vrai) != faux,
	}
}
...
func bornerDepotImport(ctx huma.Context, suite func(huma.Context)) {
	...
	if err := r.ParseMultipartForm(1 << 20); err != nil {
		...
		ecrireProblem(w, r, problem(http.StatusRequestEntityTooLarge, "IMPORT_FILE_TOO_LARGE", message))
		return
	}
	suite(ctx)
}
...
func (s *service) deposerImport(kind db.ImportKind) func(context.Context, *DepotImportInput) (*ImportJobOutput, error) {
	return func(ctx context.Context, in *DepotImportInput) (*ImportJobOutput, error) {
		...
		if !fichier.IsSet {
			return nil, problem(http.StatusBadRequest, "IMPORT_FILE_MISSING", "Aucun fichier reçu. Envoyez le classeur dans un champ `file`.")
		}
		...
		job, err := s.q.InsertImportJob(ctx, db.InsertImportJobParams{
			ID: identifiant.String(), Kind: kind, RequestedByID: utilisateurCourant(ctx).ID,
...
func ecrireClasseurImport(chemin string, source io.Reader, maximum int64) (int64, error) {
	...
	if _, err := io.ReadFull(source, tete); err != nil || !bytes.Equal(tete, []byte("PK\x03\x04")) {
		return 0, problem(http.StatusBadRequest, "IMPORT_FILE_UNREADABLE", "Ce fichier n’est pas un classeur Excel (.xlsx).")
	}
...
func (s *service) obtenirImport(ctx context.Context, in *IDImportInput) (*ImportJobOutput, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, problem(http.StatusNotFound, "IMPORT_JOB_NOT_FOUND", "Ce travail d’import n’existe pas.")
	}
	if err != nil {
		return nil, err
	}
...
func (s *service) refusAppliquerImport(ctx context.Context, id string) error {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return problem(http.StatusNotFound, "IMPORT_JOB_NOT_FOUND", "Ce travail d’import n’existe pas.")
	}
	if err != nil {
		return err
	}
	return problem(http.StatusConflict, "IMPORT_NOT_APPLICABLE", fmt.Sprintf(
...
func monterImports(api huma.API, s *service) {
	depots := []struct {
		chemin    string
		operation string
		kind      db.ImportKind
		...
		{nomProspects, "createProspectsImport", db.ImportKindPROSPECTS},
