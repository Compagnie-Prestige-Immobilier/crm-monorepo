...
func representantIntrouvable() error {
	return problem(http.StatusNotFound, "REPRESENTANT_NOT_FOUND", representantMsgAbsente)
}
...
func representantLitTout(role Role, mesFiches bool) bool {
	if mesFiches {
		return false
	}
...
var representantSensParDefaut = map[string]string{
	...
	champCreeLe:           representantTriDesc,
	representantChampNom:  representantTriDesc,
	nomProspects:          representantTriDesc,
	"lastCallAt":          representantTriDesc,
	"nextCallbackAt":      representantTriAsc,
	"priorite":            representantTriAsc,
}

var representantTriDuSuivi = map[string]string{lotEtatARappeler: "nextCallbackAt", "INJOIGNABLE": "lastCallAt"}
...
func representantFiltres(u *Utilisateur, in *RepresentantListInput) (db.ListRepresentantsParams, error) {
...
func (s *service) chercherRepresentant(ctx context.Context, in *RepresentantLookupInput) (*RepresentantLookupOutput, error) {
	phone, err := normaliserTelephone(in.Phone, s.cfg.PhoneRegion)
	if err != nil {
		return nil, err
	}
	...
	rows, err := s.q.ListRepresentants(ctx, db.ListRepresentantsParams{
		PhoneE164: &phone, ReadsEveryone: true, OwnerID: utilisateurCourant(ctx).ID,
		SortBy: representantTriDefaut, SortDir: representantTriDesc, Lim: 1,
	})
	if err != nil {
		return nil, err
...
func (s *service) verifierIdentifiantRepresentantLibre(ctx context.Context, u *Utilisateur, id string) error {
	...
	if u.Role != Admin && proprietaire != u.ID {
		return problem(http.StatusForbidden, "ENTITY_ID_OWNED_BY_ANOTHER_USER", "Cet identifiant appartient à un autre commercial.")
	}
	p := problem(http.StatusConflict, "REPRESENTANT_ALREADY_EXISTS", "Un représentant porte déjà cet identifiant.")
	p.Errors = []*huma.ErrorDetail{{Location: "body.id", Message: p.Message, Value: id}}
	return p
}

func (s *service) verifierTelephoneRepresentantLibre(ctx context.Context, u *Utilisateur, phone string) error {
	...
	p := problem(http.StatusConflict, "REPRESENTANT_PHONE_CONFLICT", "Ce numéro est déjà enregistré pour un représentant.")
...
func (s *service) conflitInsertionRepresentant(ctx context.Context, u *Utilisateur, id, phone string, err error) error {
...
func representantVerifierWhatsapp(statut db.WhatsappStatus, numero, actuel *string) error {
	if numero != nil && statut != db.WhatsappStatusAUTRENUMERO {
		return problem(http.StatusBadRequest, "WHATSAPP_NUMBER_NOT_ALLOWED", "Un numéro WhatsApp distinct n’a de sens qu’avec le statut AUTRE_NUMERO.")
	}
	if statut == db.WhatsappStatusAUTRENUMERO && numero == nil && actuel == nil {
		return problem(http.StatusBadRequest, "WHATSAPP_NUMBER_REQUIRED", "Le statut AUTRE_NUMERO exige le numéro WhatsApp.")
	}
	return nil
}
...
func representantPatchWhatsapp(cible *db.Representant, in *representantPatch, region string) error {
	...
	if in.Body.WhatsappE164 != nil {
		e164, err := normaliserTelephone(*in.Body.WhatsappE164, region)
		if err != nil {
			return err
		}
		numero = &e164
...
func (s *service) modifierRepresentant(ctx context.Context, in *RepresentantUpdateInput) (*RepresentantOutput, error) {
	...
	if u.Role != Admin && existant.CreatedById != u.ID {
		return nil, problem(http.StatusForbidden, "NOT_OWNER", "Cette fiche appartient à un autre commercial.")
	}
	modifie := existant
...
func representantAppliquerPatch(cible *db.Representant, in *representantPatch, region string) error {
	if in.Body.Phone != nil {
		phone, err := normaliserTelephone(*in.Body.Phone, region)
		if err != nil {
			return err
		}
...
func representantEcrireModification(ctx context.Context, q *db.Queries, userID string, existant, modifie *db.Representant, in *representantPatch) error {
	...
	if ecrites != 1 {
		p := problem(http.StatusConflict, "REV_CONFLICT", "La fiche a été modifiée entre-temps par un autre utilisateur. Rechargez-la avant de réessayer.")
...
var representantChampsFiche = []string{
	representantChampNom, prospectChampPrenom, representantChampTel, prospectChampEtablissement, representantChampNotes, representantChampDep, representantChampIef,
	prospectChampWhatsappStatut, prospectChampWhatsappNumero, prospectChampProfession, representantChampSyndicat, representantChampConnaitUES, representantChampContacte,
}
...
func representantJournalFiche(ctx context.Context, q *db.Queries, userID, id string, avant, apres map[string]any) error {
	...
	return auditer(ctx, q, userID, representantActionWEB, representantEntite, id, initial, change)
}
...
func (s *service) historiqueAppelsRepresentant(ctx context.Context, in *RepresentantIDInput) (*RepresentantCallAttemptsOutput, error) {
	...
	for i := range rows {
		...
		_, statErr := notes.Stat(noteNom(rows[i].ID))
		item.HasNoteVocale = statErr == nil
...
func (s *service) ajouterCommentaireRepresentant(ctx context.Context, in *RepresentantCommentInput) (*RepresentantCommentOutput, error) {
	...
	if pose.RepresentantId != in.ID || pose.AuthorId != u.ID {
		return nil, problem(http.StatusForbidden, "ENTITY_ID_OWNED_BY_ANOTHER_USER", "Cet identifiant appartient à un autre commentaire.")
	}
...
func (s *service) supprimerCommentaireRepresentant(ctx context.Context, in *RepresentantCommentDeleteInput) (*RepresentantOkOutput, error) {
	...
	if supprimes != 1 {
		return nil, problem(http.StatusNotFound, "REPRESENTANT_COMMENT_NOT_FOUND", "Commentaire introuvable.")
	}
...
func (s *service) supprimerRepresentant(ctx context.Context, in *RepresentantDeleteInput) (*RepresentantOkOutput, error) {
	...
	if prospects > 0 && !in.Cascade {
		p := problem(http.StatusConflict, "REPRESENTANT_HAS_PROSPECTS",
...
var gardeRepresentants = map[string][]Role{
	"GET /api/v1/representants":                              parcours,
	"GET /api/v1/representants/lookup":                       parcours,
	"POST /api/v1/representants":                             parcours,
	"GET /api/v1/representants/{id}":                         parcours,
	"PATCH /api/v1/representants/{id}":                       parcours,
	"DELETE /api/v1/representants/{id}":                      parcours,
	"GET /api/v1/representants/{id}/relation-history":        parcours,
	"GET /api/v1/representants/{id}/call-attempts":           parcours,
	"GET /api/v1/representants/{id}/fiche-history":           parcours,
	"GET /api/v1/representants/{id}/comments":                parcours,
	"POST /api/v1/representants/{id}/comments":               parcours,
	"DELETE /api/v1/representants/{id}/comments/{commentId}": admin,
}

func monterRepresentants(api huma.API, s *service) {
