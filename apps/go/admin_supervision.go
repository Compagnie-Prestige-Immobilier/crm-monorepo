...
func trancheDansCreneaux(tranche time.Time, creneaux []creneauEffectif) bool {
...
func (s *service) tranchesParCompte(ctx context.Context, debut time.Time, creneaux []creneauEffectif) (map[string]*totauxDeTranches, error) {
...
// les quinze secondes.
func requeteCadenceDuJour(shifts []CreneauDeTravail) string {
...
WITH attempts AS (
  SELECT "performedById" AS "userId", "clientCreatedAt", "createdAt",
    ...
    ("outcome" NOT IN ` + issuesNonJointes + `)::int AS joint,
  ...
  SELECT "performedById", "clientCreatedAt", "createdAt",
    ...
    ("outcome" IN ` + issuesRepresentantJointes + `)::int, 0
...
derniere_reponse AS (
  ...
  WHERE ` + arbitrageDeLAdhesion + ` IS NOT NULL
...
func (s *service) cadenceDuJour(ctx context.Context, debut time.Time, shifts []CreneauDeTravail) map[string]*metriquesDuJour {
...
func versCompteSupervise(c *comptePlateau, tranches *totauxDeTranches, m *metriquesDuJour, creneauxEcoules int, maintenant time.Time) CompteSupervise {
	...
	return CompteSupervise{
		...
		Score: noteDeRendement(&rendement),
	}
}
...
func (s *service) supervisionDesComptes(ctx context.Context, _ *struct{}) (*SupervisionOutput, error) {
	...
	creneaux := creneauxRabotes(shifts, &FiltreDeSupervision{})
	ecoulees := secondesDeCreneauEcoulees(jour.Format(time.DateOnly), creneaux, maintenant)
