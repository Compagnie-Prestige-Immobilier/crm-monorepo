...
func TestExportProspectsFeuillesEtInjectionDeFormule(t *testing.T) {
	...
	if len(entetes) != len(exportEntetesProspects) || entetes[0] != "Nom" || entetes[11] != "Date de saisie" {
...
func TestExportRefuseParRole(t *testing.T) {
	...
	// Le superviseur est écarté de l'export prospects, gardé sur l'annuaire.
	statut, _, _ = b.classeur("/api/v1/export/prospects.xlsx")
	b.attend(statut, http.StatusForbidden, "prospects pour un superviseur", nil)
