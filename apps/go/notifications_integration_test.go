...
func (b *banc) notificationService() *service {
	b.t.Helper()
	cfg, err := lireConfig()
	if err != nil {
		b.t.Fatal(err)
...
func TestNotificationProgrammeePuisAnnuleeNestJamaisExpediee(t *testing.T) {
	...
	if err := b.notificationService().expedierNotificationsDues(b.ctx); err != nil {
		t.Fatal(err)
	}
...
func TestNotificationDeuxExpediteursConcurrentsNenvoientQuUneFois(t *testing.T) {
	...
	for i := range 2 {
		...
		go func() {
			defer attente.Done()
			<-depart
			pris, err := s.expedierNotifications(b.ctx, []string{id}, maintenant)
...
func TestNotificationRappelQuotidienIdempotentSurLaPeriode(t *testing.T) {
	...
	for range 2 {
		if err := s.compteRenduQuotidien(b.ctx); err != nil {
			t.Fatal(err)
		}
	}
	cle := "daily-report:" + b.userID
	periode := s.jourNotification(time.Now())
	var n int
	if err := b.pool.QueryRow(b.ctx,
