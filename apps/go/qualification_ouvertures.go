...
func qualificationBorneJour(iso string, fin bool, loc *time.Location) (*time.Time, error) {
	...
	if err != nil {
		return nil, problem(http.StatusBadRequest, "BAD_REQUEST", "Date invalide : "+iso)
	}
	if fin {
...
func qualificationRappelDTO(id, prospectID, phone string, comment *string, assigneID, assigneNom string, quand, maintenant time.Time) QualificationRappelDTO {
	return QualificationRappelDTO{
		ID: id, ProspectID: prospectID, ShortCode: qualificationCodeCourt(prospectID), PhoneE164: phone,
		ScheduledAt: qualificationISO(quand), Comment: comment, AssignedToID: assigneID,
		AssignedToName: assigneNom, Overdue: quand.Before(maintenant),
	}
}
...
func (s *service) qualificationListerRappels(ctx context.Context, in *QualificationRappelsInput) (*QualificationRappelsOutput, error) {
	...
	for i := range rows {
		r := &rows[i]
		out.Body.Items = append(out.Body.Items, qualificationRappelDTO(r.ID, r.ProspectId,
			r.PhoneE164, r.Comment, r.AssignedToId, r.AssignedToName, r.ScheduledAt, maintenant))
	}
	return out, nil
}
...
func (s *service) qualificationAnnulerRappel(ctx context.Context, in *QualificationIDInput) (*QualificationRappelOutput, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, problem(http.StatusNotFound, "CALLBACK_NOT_FOUND", "Rappel introuvable.")
	}
	if err != nil {
		return nil, err
	}
	if u.Role != Admin && row.AssignedToId != u.ID {
		return nil, problem(http.StatusForbidden, "NOT_OWNER", "Ce rappel a été promis par un autre téléconseiller.")
	}
	...
	return &QualificationRappelOutput{Body: qualificationRappelDTO(row.ID, row.ProspectId, row.PhoneE164,
		row.Comment, row.AssignedToId, row.AssignedToName, row.ScheduledAt, maintenant)}, nil
}
...
func qualificationOuvertureParID(ctx context.Context, q *db.Queries, id string) (QualificationOuvertureFicheDTO, error) {
	...
	if len(rows) == 0 {
		return QualificationOuvertureFicheDTO{}, problem(http.StatusNotFound, "OUVERTURE_INTROUVABLE",
...
func (s *service) qualificationOuvrirFiche(ctx context.Context, in *QualificationOuvrirFicheInput) (*QualificationOuvertureOutput, error) {
	...
	if (b.RepresentantID == nil) == (b.ProspectID == nil) {
		return nil, problem(http.StatusBadRequest, "OUVERTURE_CIBLE_INVALIDE",
	...
	if len(rejoue) == 1 {
		if rejoue[0].OpenedById != u.ID {
			return nil, problem(http.StatusConflict, "OUVERTURE_ID_PRIS",
...
func (s *service) qualificationOuvertureConcurrente(ctx context.Context, u *Utilisateur, id string, cause error) (*QualificationOuvertureOutput, error) {
	var p *ProblemError
...
func (s *service) qualificationFicheOuvrable(ctx context.Context, u *Utilisateur, representantID, prospectID *string) error {
	introuvable := problem(http.StatusNotFound, "OUVERTURE_FICHE_INTROUVABLE",
	...
	ouvrable, err := s.q.ProspectOuvrable(ctx, db.ProspectOuvrableParams{
		ID: *prospectID, Tous: qualificationVoitTout(u.Role), Agent: u.ID,
		ConvertiVisible: u.Role == ChargeClientele,
	})
	if err != nil {
		return err
	}
...
func (s *service) qualificationBrouillonDeDepart(ctx context.Context, u *Utilisateur, representantID, prospectID *string, saisi map[string]any) ([]byte, error) {
	if saisi != nil {
...
func qualificationCreerOuverture(ctx context.Context, q *db.Queries, u *Utilisateur, in *QualificationOuvrirFicheInput, brouillon []byte) (QualificationOuvertureFicheDTO, error) {
	b := &in.Body
	...
	return QualificationOuvertureFicheDTO{}, problem(http.StatusConflict, "OUVERTURE_FICHE_DEJA_OUVERTE",
...
func qualificationBrouillonRefuse(ctx context.Context, q *db.Queries, u *Utilisateur, id string) error {
	row, err := q.OuvertureEtat(ctx, id)
	if errors.Is(err, pgx.ErrNoRows) || (err == nil && row.OpenedById != u.ID) {
		return problem(http.StatusNotFound, "OUVERTURE_INTROUVABLE",
			"Cette ouverture n’existe pas, ou elle appartient à un autre téléconseiller.")
	}
	if err != nil {
		return err
	}
	return problem(http.StatusConflict, "OUVERTURE_DEJA_FERMEE", "Cette fiche a déjà été qualifiée ou libérée.")
}
...
func (s *service) qualificationBasculerSuggestion(ctx context.Context, in *QualificationSuggestionStatutInput) (*QualificationSuggestionOutput, error) {
	u := utilisateurCourant(ctx)
	introuvable := problem(http.StatusNotFound, "SUGGESTION_NOT_FOUND", "Ce numéro suggéré est introuvable.")
	...
	if u.Role != Admin && !qualificationTransitionLegale(qualificationTransitionsSuggestion, depuis, in.Body.Status) {
		return nil, problem(http.StatusForbidden, "SUGGESTION_TRANSITION_REFUSED",
