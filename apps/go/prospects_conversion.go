...
func (s *service) prospectConvertir(ctx context.Context, in *ProspectConversionInput) (*ProspectOutput, error) {
	...
	if errors.Is(err, pgx.ErrNoRows) || journey.Consent != db.GrandPublicConsentINTERESSE {
		return nil, problem(http.StatusBadRequest, "GRAND_PUBLIC_CONSENT_REQUIRED",
...
func prospectFusion(ctx context.Context, q *db.Queries, u *Utilisateur, sourceID, targetID string, preferSource bool) error {
	...
	return auditer(ctx, q, u.ID, "prospect.merge", prospectEntite, targetID,
...
func prospectFusionnerParcours(ctx context.Context, q *db.Queries, depart, arrivee *db.JourneysAFusionnerRow, targetID string) error {
	if depart.AConversion && arrivee.AConversion {
		return problem(http.StatusConflict, "MERGE_TWO_CONVERSIONS",
...
var prospectCatalogue = []prospectChampMeta{
	...
	{prospectChampPrenom, "Prénom", prospectLesDeuxProjets, prospectSeulChues},
	{prospectChampPhone, "Téléphone", prospectLesDeuxProjets, nil},
	{prospectChampEmail, "E-mail", prospectLesDeuxProjets, nil},
	{prospectChampProfession, "Profession", prospectLesDeuxProjets, prospectSeulChues},
	...
	{prospectChampEtablissement, exportEnteteEtablissement, prospectLesDeuxProjets, nil},
	{prospectChampDureeSysteme, "Durée du système de paiement", prospectSeulGrandPublic, nil},
	{prospectChampWhatsappStatut, "Numéro WhatsApp", prospectLesDeuxProjets, nil},
	{prospectChampWhatsappNumero, "Autre numéro WhatsApp", prospectLesDeuxProjets, nil},
...
var prospectChampsImposes = []string{
	prospectChampNom, prospectChampPrenom, prospectChampPhone,
...
func (s *service) prospectMajChamps(ctx context.Context, in *ProspectMajChampsInput) (*ProspectReglagesOutput, error) {
	...
	for _, ligne := range in.Body.Champs {
		...
		if !ligne.Visible && slices.Contains(prospectChampsImposes, meta.champ) {
			return nil, problem(http.StatusBadRequest, "CHAMP_IMPOSE_MASQUE",
...
func prospectNormaliserChampLibre(entree *ProspectChampLibreEntree, connus map[string]bool) (ProspectChampLibre, error) {
	...
	if libelle == "" {
		return ProspectChampLibre{}, problem(http.StatusBadRequest, "CHAMP_LIBRE_SANS_LIBELLE",
	...
	if entree.Type == prospectTypeListe {
		...
		if len(propres) == 0 {
			return ProspectChampLibre{}, problem(http.StatusBadRequest, "CHAMP_LIBRE_SANS_VALEUR",
...
func (s *service) prospectMajParametres(ctx context.Context, in *ProspectMajParametresInput) (*ProspectParametresOutput, error) {
	...
	for _, cle := range prospectClesParametres {
		...
		if demandee && u.Role != Admin && !slices.Contains(prospectTextesPartages, cle) {
			return nil, problem(http.StatusForbidden, "PARAMETRE_RESERVE_ADMIN",
