...
func (s *service) rappelerDossiersBanqueNotification(ctx context.Context, maintenant time.Time, prefixe string, defautJours int, cle string) error {
	...
	jours, err := envInt(prefixe+"_DAYS", defautJours)
	if err != nil {
		jours = defautJours
	}
	...
	candidats, err := s.candidatsParRolesNotification(ctx, []string{string(BanqueFinance)}, map[string]string{
