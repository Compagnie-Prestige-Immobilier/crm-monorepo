...
const (
	...
	exportLibelleMauvaisNumero  = "Mauvais numéro"
...
var gardeExports = map[string][]Role{
	"GET /api/v1/export/global.xlsx":                        encadrement,
	"GET /api/v1/export/prospects.xlsx":                     {Admin, Commercial, ChargeClientele, Direction},
	"GET /api/v1/export/representants.xlsx":                 parcours,
	"GET /api/v1/export/visites.xlsx":                       registre,
	"GET /api/v1/export/bank-cases.xlsx":                    banque,
	"GET /api/v1/export/prospects-modele.xlsx":              admin,
	"GET /api/v1/export/prospects-grand-public-modele.xlsx": admin,
	"GET /api/v1/export/representants-modele.xlsx":          admin,
}

func monterExports(api huma.API, s *service) {
...
var (
	...
	exportLibellesPhase2            = map[string]string{exportCleEnAttente: "En attente", exportCleMethodeObtenue: exportLibelleMethodeObtenue, exportCleRefus: exportLibelleRefus, exportCleMauvaisNumero: exportLibelleMauvaisNumero}
	...
	exportLibellesIssue             = map[string]string{exportCleMethodeObtenue: exportLibelleMethodeObtenue, string(db.CallOutcomeUNREACHABLE): exportLibelleInjoignable, string(db.CallOutcomeCALLBACK): exportLibelleARappeler, exportCleRefus: exportLibelleRefus, exportCleMauvaisNumero: exportLibelleMauvaisNumero, string(db.CallOutcomeOTHER): exportLibelleAutre}
	...
	exportLibellesPaiement          = map[string]string{"COMPTANT": "Comptant", "ECHELONNE": "Échelonné"}
...
func exportConditionsRepresentants(u *Utilisateur, in *ExportRepresentantsInput) (*exportPredicat, error) {
	p := &exportPredicat{clauses: []string{`r."deletedAt" IS NULL`}}
	if u.Role != Admin && u.Role != Superviseur && u.Role != Direction {
	...
	if in.CommercialId != "" {
		...
		if u.Role != Admin && u.Role != Superviseur && u.Role != Direction && cible != u.ID {
...
func exportBornes(p *exportPredicat, colonne, du, au string) error {
	for _, borne := range []struct {
		...
		if err != nil {
			return problem(http.StatusBadRequest, "BAD_REQUEST", "Date de filtre invalide : "+borne.brut)
		}
...
var exportEntetesVisites = []string{
	"N° REGISTRE", enteteDate, enteteHeure, enteteNom, "TELEPHONES", "ENTREPRISE",
	"DIRECTION", enteteDestinataire, "OBJET VISITE", "COMMENTAIRES / NOTES", "SAISIE LE",
}
...
func (s *service) exportVisites(ctx context.Context, in *ExportVisitesInput) (*huma.StreamResponse, error) {
	...
	for _, borne := range []struct {
		...
		instant, err := exportBorneDeJournee(borne.brut, borne.fin)
		if err != nil {
			return nil, problem(http.StatusBadRequest, "BAD_REQUEST", "Date de filtre invalide : "+borne.brut)
		}
		*borne.cible = &instant
	}
	c, err := exportNouveauClasseur()
