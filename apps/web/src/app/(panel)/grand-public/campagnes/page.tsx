...
export const metadata: Metadata = { title: 'Campagnes Grand Public' };

export default async function LotsExportGrandPublicPage() {
  const guard = await guardRoles(['ADMIN', 'SUPERVISEUR', 'DIRECTION']);
  if (guard.status === 'anonymous') redirect('/connexion');
  if (guard.status === 'denied') redirect('/grand-public');

  // La même requête que l'état initial de la vue, sinon la clé diffère et
  // l'écran repart en chargement après l'hydratation.
  const requete = { page: 1, projet: 'GRAND_PUBLIC' } as const;
