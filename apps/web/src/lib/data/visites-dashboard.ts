...
export async function fetchDisposition(client: ApiClient = getApiClient()): Promise<Disposition> {
  return toDisposition(unwrap(await client.GET('/api/v1/visites/tableau-de-bord/disposition')));
}
...
export async function saveDisposition(
  ...
  return toDisposition(
    unwrap(
      await client.PUT('/api/v1/visites/tableau-de-bord/disposition', {
...
export async function resetDisposition(client: ApiClient = getApiClient()): Promise<Disposition> {
  unwrap(await client.DELETE('/api/v1/visites/tableau-de-bord/disposition'));
  return fetchDisposition(client);
}
...
export async function saveDefaultDisposition(
  ...
  return toDisposition(
    unwrap(
      await client.PUT('/api/v1/visites/tableau-de-bord/disposition-par-defaut', {
        body: serializeDisposition(widgets, preset),
      }),
    ),
  );
