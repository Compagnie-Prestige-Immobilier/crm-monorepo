...
test('la console traite un appel au clavier, et la fiche suivante s’ouvre seule', async ({
  page,
}) => {
  /**
   * La fiche visée est choisie sur son statut, pas sur son rang.
   *
   * La file est triée sur la date de saisie et la base de développement
   * accumule des fiches closes : partir de la première ouvrirait une fiche que
   * l'API refuse (`PHASE2_ALREADY_COMPLETED`), et l'échec accuserait le
   * clavier.
   */
  const page1 = await page.request.get('/api/v1/prospects', {
    params: { segment: 'BDD1', pageSize: '50' },
  });
  const { items } = (await page1.json()) as {
    items: { id: string; nom: string; prenom: string; phase2Status: string }[];
  };
  const ouverte = items.find((row) => row.phase2Status === 'PENDING');
  if (ouverte === undefined) {
    throw new Error(
      'Aucune fiche BDD1 en attente : la console n’a rien à traiter. Amorcez la base ' +
        '(`pnpm db:seed`) ou relancez `ensureWorkspaceFixtures`.',
    );
  }
  ...
  const fiche = page.getByRole('region', { name: 'Fiche courante' });
  const contexte = page.getByRole('region', { name: 'Contexte' });
  await expect(fiche.getByRole('heading', { level: 2 })).toHaveText(
    `${ouverte.nom} ${ouverte.prenom}`,
  );
  await expect(contexte).toContainText('0 appel consigné');

  // ─── Injoignable, touche 4 : consigné et fiche suivante ────────────────────
  await page.keyboard.press('4');
  await expect(contexte).toContainText('1 appel consigné');
  // La suivante s'ouvre SEULE : aucun bouton « suivant » n'existe, et c'est le
  // point du poste au clavier.
  await expect(fiche.getByRole('heading', { level: 2 })).not.toHaveText(
    `${ouverte.nom} ${ouverte.prenom}`,
  );
  const suivante = (await fiche.getByRole('heading', { level: 2 }).textContent())?.trim() ?? '';
  expect(suivante).not.toBe('');

  // ─── 5 ouvre les échéances, Échap les referme ──────────────────────────────
  await page.keyboard.press('5');
  await expect(fiche.getByText('Quand rappeler')).toBeVisible();
  await page.keyboard.press('Escape');
  await expect(fiche.getByText('Quand rappeler')).toHaveCount(0);

  // ─── 5 puis le chiffre d'une échéance : rappel promis, fiche suivante ──────
  // La file des rappels désigne une fiche par son NUMÉRO, pas par son nom.
  const numero = (await fiche.getByText(/^\+221 /).textContent())?.trim() ?? '';
  expect(numero).not.toBe('');

  await page.keyboard.press('5');
  await expect(fiche.getByText('Quand rappeler')).toBeVisible();
  // La première échéance proposée est « Dans 1 h » : toujours à venir, donc
  // toujours dans les six jours que couvre l'onglet « Cette semaine ».
  await page.keyboard.press('1');
  await expect(contexte).toContainText('2 appels consignés');
  await expect(fiche.getByRole('heading', { level: 2 })).not.toHaveText(suivante);

  // Le rappel promis est arrivé dans la file des rappels, et il y propose le
  // retour vers la console.
  await page.goto('/rappels');
  await expect(page.getByRole('heading', { name: 'Rappels', level: 1 })).toBeVisible();
  await page.getByRole('tab', { name: 'Cette semaine' }).click();
  const ligne = page.getByRole('row').filter({ hasText: numero });
  await expect(ligne).toHaveCount(1);
  await expect(ligne.getByRole('link', { name: 'Ouvrir dans la console' })).toBeVisible();
});
