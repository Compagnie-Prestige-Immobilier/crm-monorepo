...
test('CHU-DSP-05 · le tiroir dit quand tout est placé', async ({ page }) => {
  ...
  for (const source of SOURCES_EN_RESERVE) {
    await tiroir.getByRole('button', { name: new RegExp(`^${source}`) }).click();
  }

  await expect(tiroir).toContainText('Toutes les sources sont déjà placées.');
  await expect(tiroir.getByRole('button', { name: /^Méthodes d’adhésion/ })).toHaveCount(0);
...
test('CHU-DSP-07 · la marque d’une carte se change et survit au rechargement', async ({ page }) => {
  ...
  /**
   * Une jauge est un graphique : elle pose un `canvas` dans la région de la
...
test('CHU-DSP-09 · « Quitter » avec des changements demande confirmation', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Retirer Rappels promis' }).click();
  await page.getByRole('button', { name: 'Quitter' }).click();

  const confirmation = page.getByRole('dialog');
  await expect(confirmation.getByText('Quitter sans enregistrer', { exact: true })).toBeVisible();
  ...
  await expect(carte(page, 'Rappels promis')).toBeVisible();
});
