...
void main() {
  ...
  group('motifs d\'issue', () {
    ...
    test('un refus DEFINITIF abandonne la note et laisse partir l’appel', () async {
      // Le fichier vide ou le type non audio ne guerissent pas. Les remettre en
      // file bloquait la cloture de l'appel pour toujours: le teleconseiller
      // avait bien passe son appel, et rien ne remontait.
      await seedDirectory();
      final File recording = File(
      ...
      expect(
        ...
        isFalse,
        reason: 'le fichier refuse est retire',
      );
      expect(
        (await allOutbox(db)).single.status,
        OutboxStatus.done,
        reason: 'l’appel part malgre le refus de sa note',
      );
    });
