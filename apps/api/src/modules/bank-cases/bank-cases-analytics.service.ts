...
export class BankCaseAnalyticsService {
  ...
  async totals(filter: BankCaseFilterDto, demoEnabled?: boolean): Promise<BankAnalyticsTotalsDto> {
    const where = bankCaseConditions(filter, demoEnabled ?? (await this.demo.enabled()));
  ...
  async byStage(filter: BankCaseFilterDto, demoEnabled?: boolean): Promise<BankStageCountDto[]> {
    const where = bankCaseConditions(filter, demoEnabled ?? (await this.demo.enabled()));
    const rows = await this.prisma.$queryRaw<
      {
        stageId: string;
  ...
  async createdOverTime(
    ...
    const where = bankCaseConditions(query, demoEnabled ?? (await this.demo.enabled()));
  ...
  async byBank(filter: BankCaseFilterDto, demoEnabled?: boolean): Promise<BankBankBreakdownDto[]> {
    const where = bankCaseConditions(filter, demoEnabled ?? (await this.demo.enabled()));
    const rows = await this.prisma.$queryRaw<
      {
        banqueId: string;
  ...
  async byAgent(filter: BankCaseFilterDto, demoEnabled?: boolean): Promise<BankAgentActivityDto[]> {
    const where = bankCaseConditions(filter, demoEnabled ?? (await this.demo.enabled()));
    const rows = await this.prisma.$queryRaw<
      {
        agentId: string;
