...
describe('dayIndexFor', () => {
  ...
  it('découpe en tranches CONTIGUËS : le jour 2 suit le jour 1', () => {
    // Un second tourniquet mêlerait les journées, et le programme du jour 3 ne
    // reprendrait pas là où celui du jour 2 s'arrête.
  ...
  it('RÉPARTIT le reste au lieu de l’empiler sur la dernière journée', () => {
    // 10 lignes sur 3 jours donnent 4, 3, 3, et 100 sur 7 donnent deux journées
    // à 15 réparties dans la semaine. Une division entière suivie d'un reliquat
    // donnerait 14 partout puis 16 le dernier jour, c'est-à-dire la journée la
    // plus chargée juste avant la clôture de la campagne.
    expect(dayHistogram(10, 3)).toEqual([4, 3, 3]);
    expect(dayHistogram(100, 7)).toEqual([15, 14, 14, 15, 14, 14, 14]);
  });

  // Régression : l'aperçu rendait `min(spreadDays, count)` cases quand le
  // détail d'une campagne en rend `spreadDays`. Les deux publient un champ
  // `perDay`, si bien qu'un graphique tracé depuis l'aperçu puis redessiné
  // depuis le détail CHANGEAIT DE LONGUEUR sans qu'aucune donnée n'ait bougé.
  // La série fait donc toujours `spreadDays` cases, journées vides comprises.
  it('rend toujours autant de cases que de journées demandées', () => {
    // Trois lignes sur sept jours : le travail tient en trois journées, mais
    // la campagne en compte bien sept, et les quatre dernières sont vides.
  ...
  // La répartition, elle, ne s'étale pas au-delà de ce qu'elle peut remplir :
  // trois lignes sur sept jours restent une par jour sur trois jours, jamais
  // une ligne isolée le septième.
