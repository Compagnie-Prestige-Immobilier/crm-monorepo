...
export function GlobalExportButton() {
  ...
  async function download(): Promise<void> {
    setPending(true);
    try {
      const response = await fetch('/api/v1/export/global.xlsx', { cache: 'no-store' });
      if (!response.ok) {
  ...
  return (
    <Button
      variant="outline"
      className="cpi-export border-white/45 bg-white text-primary hover:bg-white/90 hover:text-primary disabled:border-white/45 disabled:bg-white disabled:text-primary/60"
      disabled={pending}
      aria-busy={pending}
