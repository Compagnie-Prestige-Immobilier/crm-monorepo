...
export const Route = createFileRoute('/_panneau/chues/dossiers/$id')({
...
/** La page `(panel)/chues/dossiers/[id]` de la v1. */
function DossierPage() {
  const { user } = Route.useRouteContext();
  const { id } = Route.useParams();

  return <BankCaseDetailView caseId={id} role={user.role} projet="CHUES" />;
}
