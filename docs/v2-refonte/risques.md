...
| R5 | `drizzle-kit pull` ne lit ni CHECK, ni index partiels, ni index fonctionnel | drizzle-orm #3520, #6145, #5224 ; 20 CHECK, 12 index partiels (`audits/donnees-infra.md` §2) | Réécriture manuelle, preuve D par diff SQL contre la base | 0 | couvert |
...
| R9 | Sept crons et le WebSocket de présence sans hôte dans Next pur | `imports.cron.ts:44`, `reminders.service.ts:96,137,191`, `db-dump.service.ts:110`, `recordings.service.ts:35`, `enrolement.service.ts:188`, `bootstrap.ts:78` | Serveur Node personnalisé | 1 | couvert |
...
| R12 | Deux identités si les tables Better Auth vivent à côté de `users` | `schema.prisma:301-330` (dizaines de FK) | `users` réutilisée via `modelName`/`fields` | 1 | couvert |
...
| R16 | Rejeu de refresh token plus détecté | `auth.service.ts:119-128` | Sessions Better Auth en base, accepté | 1 | couvert |
...
| R24 | Exports Excel en flux Node (`WorkbookWriter` vers `Writable`) contre `ReadableStream` Web dans un route handler | `export.service.ts:199-231` | Pont `Readable.toWeb`, testé sur l'export `consolidated` à 500 000 lignes | 2 | ouvert |
...
| R26 | 223 sites d'autorisation à reporter sans en perdre un | `audits/critique-plan.md` §2.2 | Table de garde par route dans `lib/roles`, seizième parcours « matrice des rôles » généré depuis cette table | 1, 7 | couvert |
...
| R28 | Invalidation de cache couplée au SSE : un handler qui oublie `bump` perd une invalidation | `redis/cache.interceptor.ts:58-62` | Une fonction `invalidate(group)` unique appelée par les handlers d'écriture ; parcours Playwright sur les référentiels | 1 | couvert |
...
| R35 | Ports de développement des deux worktrees en collision | `plan.md` §0 | v2 sur 4000, 5435, 6382, 8080 ; `.env.example` de `apps/go` | 1 | couvert |
| R36 | Colonnes mortes (`users.departementId`, `device_tokens`, `notifications.payload`) reprises telles quelles par `pull` | `docs/migrations-en-attente.md` §1-2 | Migration de suppression en phase 0, avec la release v1 | 0 | couvert |
...
| SEC-02 : rôle jamais réévalué après connexion | `AuthState.role` | Rôle dans le JWT d'une heure, session Better Auth |
| SEC-04 : rejeu du refresh après timeout | `auth_interceptor.dart` | Sessions Better Auth, plus de rotation maison |
