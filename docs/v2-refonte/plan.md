...
| Bases multiples | Une base Postgres par `DATABASE_URL_<NOM>` sur le même serveur, une instance complète (pool, API, SSE, crons) par base, cookie `cpi_base` choisi sur la page de connexion. Rien de « démo » dans le code métier ; le dossier des dumps reste partagé, décision du propriétaire | 10 septembre |
| Notes vocales | Retirées le 10 septembre sur ordre du propriétaire : inutilisées, et leur balayage de fichiers orphelins interdisait un dossier partagé entre bases. Décision initiale conservée pour mémoire. Conservées : une note dictée au micro après l'appel, jamais l'audio de l'appel lui-même, qu'un navigateur ne peut pas capter. Enregistrées par `MediaRecorder`, mêmes routes que la v1, conteneurs `audio/webm` et `audio/mp4`, 2 minutes, 48 h ; périmètre conversion, comme le mobile. Libellé d'interface : « note vocale », jamais « enregistrement d'appel ». L'enregistrement des appels est hors périmètre : il exigerait une téléphonie par API | 8 septembre |
