...
## Questions au proprietaire

Ne poser une question que si le code, la demande et les decisions deja ecrites
ne permettent pas de trancher. Une migration, un refactoring ou un portage
reproduit le comportement actuel a l'identique, incoherences comprises : ne
jamais demander s'il faut les corriger en passant. Les signaler en une ligne
dans le rapport, sans question, et continuer.
...
Le fichier `.yolo` est un garde-fou local : il doit exister a la racine, etre
ignore par Git et contenir un secret non vide choisi par le proprietaire. Ne
jamais lire, afficher, copier, ajouter ou commiter son contenu.

Si ce fichier est present et que le proprietaire formule une instruction
explicite terminee par `YOLO`, l'agent execute cette instruction sans nouvelle
demande de confirmation ni objection fondee sur les conventions du depot. Cela
peut notamment couvrir un deploiement de production ou une desactivation
